...
import weakref  # FIX-LIFECYCLE: 弱引用避免循环引用
...
"""
    ...
    def scan_file(self, path):
        ...
        try:
            ...
            # FIX-SECURITY: 先检查哈希黑名单
            try:
                hash_checker = HashBlacklist()
                hash_result = hash_checker.check_file(path)
            ...
            # FIX-SECURITY: PE文件分析（可疑度评分）
            try:
                if path.lower().endswith(('.exe', '.dll', '.sys', '.com', '.scr')):
...
class LiangDunApp(tk.Tk):
    ...
    def _on_close(self):
        ...
        # 取消所有 pending 的 after 回调
        try:
            for attr_name in dir(self):
                if attr_name.startswith('_') and 'timer' in attr_name.lower():
    ...
    def _on_gpu_accel_toggle(self):
        ...
        if self._gpu_accel_var.get():
            # FIX-FEATURE: 根据GPU类型配置不同的加速策略
            if gpu_type == "nvidia":
                # NVIDIA GPU 可以使用 CUDA 加速
                self._toast.show("GPU 加速已启用", 
                                f"将使用 {gpu_name} (CUDA) 加速扫描",
                                icon_type="success")
                self._log(f"GPU 加速已启用: {gpu_name} (CUDA模式)", "success")
                # 设置环境变量启用GPU加速（如果ClamAV支持）
                os.environ["CLAMAV_GPU_ACCEL"] = "1"
            elif gpu_type == "apple":
                # Apple Silicon 使用 Metal 加速
                self._toast.show("GPU 加速已启用", 
                                f"将使用 {gpu_name} (Metal) 加速扫描",
                                icon_type="success")
                self._log(f"GPU 加速已启用: {gpu_name} (Metal模式)", "success")
                os.environ["CLAMAV_GPU_ACCEL"] = "1"
            else:
                # 其他GPU使用OpenCL
                self._toast.show("GPU 加速已启用", 
                                f"将使用 {gpu_name} (OpenCL) 加速扫描",
                                icon_type="success")
                self._log(f"GPU 加速已启用: {gpu_name} (OpenCL模式)", "success")
                os.environ["CLAMAV_GPU_ACCEL"] = "1"
        else:
            ...
            # 清除GPU加速环境变量
            os.environ.pop("CLAMAV_GPU_ACCEL", None)
...
def ensure_isdk_available():
    if not IS_WIN:
        return False
    if not ISWORD_DLL.exists() or not ISWORD_SYS.exists():
        logger.warning("iSDK 文件缺失: %s / %s", ISWORD_DLL.name, ISWORD_SYS.name)
        return False
    try:
        if hasattr(os, "add_dll_directory"):
            os.add_dll_directory(str(ISDK_BIN_DIR))
        ctypes.WinDLL(str(ISWORD_DLL))
        logger.info("iSDK 已加载: %s", ISWORD_DLL.name)
        return True
    except Exception as e:
...
def enable_self_protection():
    """FIX-SECURITY: Windows 下设置进程自我保护，禁止非管理员终止
    实际实现：设置进程DACL限制终止权限
    """
    if not IS_WIN:
        return False
    try:
        ...
        if not hProcess:
            logger.warning("无法获取进程句柄进行自我保护")
            return False

        try:
            # 定义安全描述符结构
            # 创建一个只允许当前用户和管理员访问的DACL
            # SECURITY_DESCRIPTOR_REVISION = 1
            # DACL_SECURITY_INFORMATION = 4
            
            # 简化实现：设置进程为关键进程（需要SeDebugPrivilege）
            # 如果没有权限，则使用注册表保护
            try:
                # 尝试设置进程为保护进程（Windows 8.1+）
                # ProcessBreakOnTermination = 29
                class ULONG(_ctypes.c_ulong):
                    pass
                    
                # 设置进程保护标志
                # 这需要管理员权限，如果失败则回退到其他方法
                result = kernel32.SetProcessShutdownParameters(0x3FF, 0)
                if result:
                    logger.info("自我保护: 进程已设置为高优先级关闭保护")
            except Exception:
                pass
            ...
            try:
                ...
                try:
                    with winreg.CreateKey(winreg.HKEY_CURRENT_USER, key_path) as key:
                        winreg.SetValueEx(key, "ProtectedProcess", 0, winreg.REG_DWORD, 1)
