...
v6.2.0 修复清单：
  FIX-THREAD-SAFE: 使用queue.Queue实现线程安全的UI更新
  FIX-LIFECYCLE: 完善退出/生命周期管理，确保所有资源正确清理
  FIX-LOG-SYNC: 日志与状态同步，UI操作只在主线程执行
  FIX-FEATURE: 完善GPU加速、YARA规则扫描、哈希黑名单等功能
  FIX-SECURITY: 增强安全功能实现（PE分析、哈希检测、自我保护）
...
import atexit
import sqlite3
import queue  # FIX-THREAD-SAFE: 线程安全消息队列
import random  # FIX-SCAN-5: 移到文件顶部，避免循环内 import
from pathlib import Path
from datetime import datetime
from concurrent.futures import ThreadPoolExecutor, as_completed  # FIX-RESOURCE: 统一线程池管理
...
def get_base_dir():
    ...
    Nuitka --onefile 解压到临时目录运行，sys.executable 指向临时目录中的 exe，
    必须通过 sys.argv[0] 获取原始 exe 路径。
    """
    # FIX-BUG-32: Nuitka onefile 设置 frozen=True 但 sys.argv[0] 仍指向原始 exe
    # 优先尝试 sys.argv[0]（对 Nuitka 和开发环境都有效）
    argv0 = sys.argv[0]
    if argv0:
        exe_path = Path(argv0)
        if exe_path.is_file():
            return exe_path.parent
        # Nuitka onefile 的 argv[0] 可能指向目录（启动脚本）
        if exe_path.is_dir() and (exe_path / 'main.py').exists():
            return exe_path
    # 兜底：PyInstaller 环境使用 sys.executable
...
ISDK_DIR = BASE_DIR / "iSDK"
ISDK_BIN_DIR = ISDK_DIR / "bin"
ISWORD_DLL = ISDK_BIN_DIR / ("iSword64.dll" if sys.maxsize > 2**32 else "iSword.dll")
ISWORD_SYS = ISDK_BIN_DIR / ("iSword64.sys" if sys.maxsize > 2**32 else "iSword.sys")

# ══════════════════════════════════════════════
#  v7.0.0 新增：iMonitor HIPS 内核驱动 SDK
# ══════════════════════════════════════════════
IMONITOR_DIR      = BASE_DIR / "iMonitor"
IMONITOR_BIN_DIR  = IMONITOR_DIR / "bin"
IMONITOR_DLL      = IMONITOR_BIN_DIR / ("iMonitor64.dll" if sys.maxsize > 2**32 else "iMonitor.dll")
IMONITOR_SYS      = IMONITOR_BIN_DIR / ("iMonitor64.sys" if sys.maxsize > 2**32 else "iMonitor.sys")
IMONITOR_RULE_FILE = CONF_DIR / "iMonitor.rule"
...
def detect_gpu():
    ...
    if IS_MAC:
        try:
            ...
            logger.debug("macOS GPU检测异常: %s", e)
        ...
        try:
            ...
            if proc.returncode == 0 and proc.stdout.strip():
                ...
                try:
                    ...
                    logger.debug("GPU JSON解析失败: %s", e)
        except Exception as e:
            logger.debug("Windows GPU检测异常: %s", e)
    ...
    try:
        ...
        if IS_WIN:
            try:
                ...
                logger.debug("Windows OpenCL检测失败: %s", e)
        elif platform.system() == "Linux":
            try:
                opencl = ctypes.CDLL("libOpenCL.so.1")
                result["opencl_available"] = True
            except Exception as e:
                logger.debug("Linux OpenCL检测失败: %s", e)
    except Exception as e:
        logger.debug("OpenCL检测异常: %s", e)
...
def _get_last_audit_hash() -> str:
    """FIX-SECURITY: 从审计日志文件读取最后一条记录的哈希，确保跨进程链式验证不中断"""
    try:
        ...
        for line in reversed(lines):
            ...
            if line:
                try:
                    ...
                    # 重新计算哈希以验证完整性
                    test_entry = {k: v for k, v in entry.items() if not k.startswith('_')}
                    test_line = json.dumps(test_entry, ensure_ascii=False, sort_keys=True)
                    expected_hash = hmac.new(
                        b"liangdun_audit_key_v1",
                        test_line.encode("utf-8"),
                        hashlib.sha256
                    ).hexdigest()[:64]  # FIX-SECURITY: 使用完整64位而非截断32位
                    # 返回存储的哈希（如果存在）
        ...
        try:
            ...
            if AUDIT_LOG.exists() and AUDIT_LOG.stat().st_size > AUDIT_LOG_MAX_SIZE:
                ...
                try:
                    ...
                    pass
            # FIX-SECURITY: 启动时从文件恢复最后一条哈希，避免链式断裂
            if _audit_prev_hash == "":
                _audit_prev_hash = _get_last_audit_hash()
            # 添加链式哈希
            entry["_prev_hash"] = _audit_prev_hash
            entry["_timestamp"] = datetime.now().isoformat()
            line = json.dumps(entry, ensure_ascii=False, sort_keys=True)
            # FIX-SECURITY: 使用完整 SHA256 哈希（64位）而非截断32位
            entry_hash = hmac.new(
                b"liangdun_audit_key_v1",
                line.encode("utf-8"),
                hashlib.sha256
            ...
            final_line = json.dumps(entry, ensure_ascii=False, sort_keys=True) + '\n'
...
class ScanHistory:
    ...
    def get_stats(self):
        """
        获取扫描统计信息
        :return: 字典包含 total_scans, total_scanned, total_infected, total_errors, week_scans
        """
        with self._lock:
            try:
                conn = self._get_connection()
                cursor = conn.cursor()
                # FIX-BUG-8: 同时获取总扫描数和本周扫描数
                cursor.execute("""
                    SELECT
                ...
                cursor.execute("""
                    SELECT COUNT(*) FROM scan_history WHERE timestamp > ?
                """, (week_ago,))
                ...
                return {
                    ...
                    "week_scans":    week_row[0] if week_row else 0,
                }
            except Exception as e:
                logger.error(f"获取扫描统计失败: {e}")
                return {"total_scans": 0, "total_scanned": 0, "total_infected": 0, "total_errors": 0, "week_scans": 0}
    ...
    def add_schedule(self, name, target, scan_type, interval_type, enabled=True):
        ...
        with self._lock:
            try:
                ...
                task_id = None
            # FIX-SAFETY: 在锁内调用 save()，确保内存和磁盘数据一致性
            try:
                self.save()
            except Exception:
...
class FileMonitor:
    ...
    def __init__(self, backend, log_cb=None):
        ...
        self._stopped = False  # FIX-HIPS-4: 标记是否已停止，防止停止后重新创建线程池
    ...
    def _process_pending_scans(self):
        ...
        for i, fpath in enumerate(batch):
            if self._stop_event.is_set():
                # FIX-SCAN-4: 使用索引而非 batch.index()，避免元素不存在时丢失所有剩余项
                remaining = batch[i:]
                if remaining:
                    with self._scan_lock:
    ...
    def scan_file(self, path):
        ...
        try:
            ...
            # FIX-SECURITY: 大文件不再静默跳过，改为记录并尝试哈希扫描
            try:
                size = os.path.getsize(path)
                if size > 100 * 1024 * 1024:
                    self._log(f"文件过大 ({size/1024/1024:.1f}MB)，尝试哈希快速检测：{_truncate_path(path)}", "warn")
                    # 对大文件至少做哈希黑名单检查
                    try:
                        if self._backend._hash_checker is None:
                            ...
                            self._log(f"大文件哈希: {file_hash}", "dim")
                    except Exception:
                        pass
                    return
            except OSError:
            ...
            try:
                ...
                # FIX-SECURITY: 处理 scan_failed 状态，避免将失败误判为安全
                if result.get("scan_failed"):
                    self._log(f"实时扫描失败: {result.get('error', '未知错误')} - {_truncate_path(path)}", "warn")
    ...
    def stop(self):
        """FIX-LIFECYCLE: 停止文件监控（完整资源清理）"""
        self._stop_event.set()
        self._running = False
        self._stopped = True  # FIX-HIPS-4: 标记为已停止
        ...
        # FIX-RESOURCE: 正确关闭扫描线程池
        # FIX-HIPS-4: DLL 函数签名可能不匹配，增加异常处理
        if self._scan_executor is not None:
            try:
                self._scan_executor.shutdown(wait=True)
            except Exception as e:
                self._log(f"线程池关闭异常: {e}", "warn")
            finally:
...
class StartupAuditor:
    ...
    def _audit_registry_run(self) -> list:
        ...
        try:
            ...
            # FIX-BUG-19: 拖拽功能需要 tkinterdnd2 库
            # FIX-HIPS-3: self.log 应为 self._log（AttributeError）
            self._log("拖拽扫描功能不可用: 缺少 tkinterdnd2 库 (pip install tkinterdnd2)", "warn")
        except Exception as e:
...
class YARARuleManager:
    ...
    def scan_with_rules(self, file_path: str) -> list:
        """FIX-FEATURE: 使用YARA规则扫描文件
        ...
        try:
            ...
            try:
                ...
                with open(p, 'rb') as f:
                    # FIX-SECURITY: 超过10MB的文件分段扫描，避免内存问题
                    if file_size > 10 * 1024 * 1024:
                        # 读取前10MB + 后5MB，中间部分采样
                        content = f.read(10 * 1024 * 1024)  # 前10MB
                        # 对于超大文件，记录警告
                        logger.warning(f"YARA扫描: 文件过大 ({file_size/1024/1024:.1f}MB)，仅扫描前10MB - {file_path}")
                    else:
                        content = f.read()
            except Exception:
                return matches
            ...
            with self._lock:
                for rule in self._rules:
                    ...
                    try:
                        # 支持十六进制签名
                        if all(c in '0123456789ABCDEFabcdef' for c in sig if c != ' '):
            ...
            logger.debug(f"YARA规则扫描失败: {e}")
        return matches


# ══════════════════════════════════════════════
#  FIX-SECURITY: 哈希黑名单/白名单检测器
# ══════════════════════════════════════════════
class HashBlacklist:
    """FIX-SECURITY: 哈希黑名单/白名单检测器"""
    
    def __init__(self):
        self._blacklist_file = USER_DATA_DIR / "hash_blacklist.json"
        self._whitelist_file = USER_DATA_DIR / "hash_whitelist.json"
        self._lock = threading.Lock()
        self._blacklist = {}  # hash -> {"name": str, "added": str}
        self._whitelist = {}  # hash -> {"name": str, "added": str}
        self.load()
        ...
        with self._lock:
            ...
            try:
                if self._blacklist_file.exists():
                    self._blacklist = json.loads(self._blacklist_file.read_text(encoding="utf-8"))
                else:
                    self._blacklist = {}
            except Exception:
                self._blacklist = {}
            ...
            try:
                if self._whitelist_file.exists():
                    self._whitelist = json.loads(self._whitelist_file.read_text(encoding="utf-8"))
                else:
                    self._whitelist = {}
            except Exception:
                self._whitelist = {}
        ...
        with self._lock:
            try:
                ...
                self._blacklist_file.write_text(
                    json.dumps(self._blacklist, ensure_ascii=False, indent=2),
                    encoding="utf-8"
                )
            except Exception as e:
    ...
    def add_to_blacklist(self, hash_value: str, name: str = "") -> bool:
        """添加哈希到黑名单"""
        with self._lock:
            self._blacklist[hash_value] = {
                "name": name,
    ...
    def add_to_whitelist(self, hash_value: str, name: str = "") -> bool:
        """添加哈希到白名单"""
        with self._lock:
            self._whitelist[hash_value] = {
                "name": name,
                "added": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            }
        self.save()
    ...
    def remove_from_blacklist(self, hash_value: str) -> bool:
        """从黑名单移除"""
        with self._lock:
            if hash_value in self._blacklist:
                del self._blacklist[hash_value]
                self.save()
                return True
        return False
        
    def remove_from_whitelist(self, hash_value: str) -> bool:
        """从白名单移除"""
        with self._lock:
            if hash_value in self._whitelist:
                del self._whitelist[hash_value]
                self.save()
                return True
        return False
    ...
    def check_file(self, file_path: str) -> dict:
        """检查文件是否在黑名单或白名单中
        :return: {"status": "blacklisted"/"whitelisted"/"unknown", "hash": str, "info": dict}
        """
        result = {"status": "unknown", "hash": "", "info": None}
        ...
        with self._lock:
            ...
            if file_hash in self._whitelist:
                ...
                result["info"] = self._whitelist[file_hash]
                return result
                
            # 再检查黑名单
            if file_hash in self._blacklist:
                result["status"] = "blacklisted"
                result["info"] = self._blacklist[file_hash]
                return result
    ...
    def get_blacklist(self) -> list:
        """获取黑名单列表"""
        with self._lock:
            return [{"hash": k, **v} for k, v in self._blacklist.items()]
            
    def get_whitelist(self) -> list:
        """获取白名单列表"""
        with self._lock:
            return [{"hash": k, **v} for k, v in self._whitelist.items()]
...
class QuarantineManager:
    ...
    def _secure_delete(self, path: Path) -> bool:
        """安全删除文件，优先使用 iSDK 内核驱动强制删除"""
        # 优先尝试 iSDK 强制删除（可处理被占用/锁定的文件）
        if isdk_is_available():
            try:
                if isdk_force_delete_file(path):
                    self.log(f"文件已强制删除: {path}", "info")
                    return True
            except Exception as e:
                self.log(f"iSDK 强制删除失败，回退标准删除: {e}", "warn")
    ...
    def quarantine_file(self, src_path: str, threat_name: str = "Unknown") -> tuple:
        ...
        # FIX-SAFETY: 先尝试重命名 tmp → qfile，成功后再删除源文件
        # 避免先删源文件后重命名失败导致文件丢失
        ...
        try:
            ...
            # 重命名失败，源文件保留不删除，避免文件丢失
            return False, f"隔离失败（重命名异常），源文件已保留: {e}"

        # 重命名成功，现在安全删除源文件
        deleted = self._secure_delete(src)
        if not deleted:
            self.log(f"警告: 隔离文件已保存但源文件删除失败（可能被占用）: {src}", "warn")
            entry["note"] = entry.get("note", "") + " 源文件删除失败，可能仍存在于原位置"
    ...
    def restore_item(self, qid: str) -> tuple:
        ...
        try:
            ...
            if cross_fs:
                ...
                # FIX-SECURITY: 跨文件系统恢复后验证复制成功再删除隔离文件
                if not orig.exists():
                    return False, "跨文件系统复制失败，隔离文件保留"
                if os.path.getsize(orig) != os.path.getsize(qfile):
                    orig.unlink()
                    return False, "跨文件系统复制不完整，隔离文件保留"
                qfile.unlink()
            else:
            ...
            # FIX-SECURITY: 恢复失败时不删除原文件，保留隔离文件
...
class ClamAVBackend:
    def __init__(self, log_cb):
        ...
        self._total_bytes = 0
        # FIX-RACE: 扫描锁，防止多目标并发时 _scan_proc / _scan_cancel 被覆盖
        self._scan_lock = threading.Lock()
        # FIX-PERF: 共享 HashBlacklist 实例，避免每次 scan_file 都重新加载
        self._hash_checker = None
...
"""
    ...
    def verify_database_integrity(self):
        ...
        for f in CVD_FILES:
            ...
            try:
                size = p.stat().st_size
                # FIX-SECURITY: 正确校验 CVD 文件完整性
                # CVD 格式: 头部(512B) = 文本头 + 填充，尾部有数字签名
                ...
                try:
                    ...
                    if len(parts) >= 5:
                        ...
                        # FIX-SECURITY: 计算整个文件（除尾部签名外）的 MD5，而非仅头部
                        # CVD 文件结构: 头部(512B) + 压缩数据 + 签名(16B)
                        actual_md5 = ""
                        try:
                            with open(p, 'rb') as fp:
                                data = fp.read()
                            # 签名在文件末尾 16 字节（如果文件足够大）
                            data_for_md5 = data[:-16] if len(data) > 528 else data
                            actual_md5 = hashlib.md5(data_for_md5).hexdigest()
                        except Exception:
                            pass

                        md5_match = (stored_md5.lower() == actual_md5.lower()) if stored_md5 and actual_md5 else None
                        results.append({
                            "name": f, "ok": True, "size": size,
    ...
    def update_database(self, progress_cb, done_cb):
        def run():
            ...
            try:
                ...
                # FIX-UPDATE-PIPE: Windows 不支持 select 对管道，改用线程+队列读取 stdout
                            ...
                            while True:
                                ...
                                if line:
                                    ...
                                    if "Downloading" in line and "%" in line:
                                        rm = re.findall(r'(\d+)%', line)
                                        if rm:
                                            rp = int(rm[-1])
                                            if 0 <= rp <= 100 and rp >= last_valid_pct - 5:
                                                pct = 20 + rp * 0.75
                                                last_valid_pct = rp
        ...
        if self._scan_proc and self._scan_proc.poll() is None:
            try:
                ...
                try:
                    ...
                    pass
        # FIX-SCAN-LOCK: 取消后清理 _scan_proc 引用，避免阻塞后续扫描
        self._scan_proc = None

    def scan_file(self, path):
        """
        FIX-BUG-2: 扫描单个文件（用于实时文件监控）
        FIX-SECURITY: 增强安全检测（哈希黑名单、PE分析）
        :param path: 文件路径
        :return: 扫描结果字典，包含 infected, virus, scan_failed 等字段；
                 失败返回 {"infected": False, "scan_failed": True, "error": ...}
                 绝不返回 None，避免调用方误判为安全
        """
        if not self.check_database():
            # FIX-SECURITY: 数据库未就绪时返回明确失败状态，而非 None
            return {"infected": False, "scan_failed": True, "error": "病毒库未安装", "path": path}
        try:
            path_obj = Path(path)
            if not path_obj.exists() or not path_obj.is_file():
                return {"infected": False, "scan_failed": True, "error": "文件不存在", "path": path}
                
            # FIX-SECURITY: 先检查哈希黑名单（使用共享实例避免重复磁盘 I/O）
            try:
                if self._hash_checker is None:
                    self._hash_checker = HashBlacklist()
                hash_result = self._hash_checker.check_file(path)
                if hash_result["status"] == "blacklisted":
                    info = hash_result.get("info", {})
                    return {
                        "infected": True,
                        "virus": f"Hash.Blacklist.{info.get('name', 'Unknown')}",
                        "path": path,
                        "detection": "hash_blacklist",
                        "hash": hash_result.get("hash", ""),
                    }
                elif hash_result["status"] == "whitelisted":
                    # 白名单文件，跳过扫描
                    return {"infected": False, "virus": None, "path": path, "whitelisted": True}
            except Exception as e:
                logger.debug("scan_file: 哈希检查异常: %s", e)
            try:
                # FIX-SCAN-1: path 可能是 Path 对象，需要转为 str 再调用 .lower()
                path_str = str(path)
                if path_str.lower().endswith(('.exe', '.dll', '.sys', '.com', '.scr')):
                    pe_result = PEProfileAnalyzer.analyze(path)
                    if pe_result.get("suspicious_score", 0) >= 70:
                        return {
                            "infected": True,
                            "virus": f"Suspicious.PE.Score{pe_result['suspicious_score']}",
                            "path": path,
                            "detection": "pe_analysis",
                            "pe_info": pe_result,
                        }
            except Exception as e:
                logger.debug("scan_file: PE分析异常: %s", e)
            ...
            try:
                # FIX-BUG-13: 增加超时时间到 300 秒（5分钟），支持大文件扫描
                stdout, _ = proc.communicate(timeout=300)
            except Exception as e:
                # FIX-SAFETY: 确保进程和管道在任何异常下都被清理
                try:
                    proc.kill()
                    proc.communicate(timeout=5)
                except Exception:
                    pass
                # FIX-SECURITY: 超时/异常时返回明确失败状态，而非 None
                return {"infected": False, "scan_failed": True, "error": f"扫描异常: {e}", "path": path}
            ...
            for line in output.splitlines():
                if "FOUND" in line:
                    ...
                    if len(parts) >= 2:
                        ...
                        return {"infected": True, "virus": virus_name, "path": path, "detection": "clamav"}
            # 未发现威胁
            return {"infected": False, "virus": None, "path": path}
        except Exception as e:
            logger.debug(f"scan_file error: {e}")
            # FIX-SECURITY: 任何异常都返回明确失败状态，绝不返回 None
            return {"infected": False, "scan_failed": True, "error": str(e), "path": path}
    ...
    def scan(self, target, progress_cb, result_cb, log_file_cb=None, exclusions=None, scan_archives=False):
        def run():
            # FIX-RACE: 获取扫描锁，确保同一时刻只有一个 scan() 在操作 _scan_proc / _scan_cancel
            if not self._scan_lock.acquire(blocking=False):
                result_cb(None, "扫描引擎忙，请等待当前扫描完成")
                return
            try:
                self._do_scan(target, progress_cb, result_cb, log_file_cb, exclusions, scan_archives)
            finally:
                self._scan_lock.release()

        threading.Thread(target=run, daemon=True).start()

    def _do_scan(self, target, progress_cb, result_cb, log_file_cb=None, exclusions=None, scan_archives=False):
        """实际执行扫描逻辑（已持有 _scan_lock，在子线程中运行）"""
        if not self.check_database():
            result_cb(None, "病毒库未安装，请先更新病毒库")
            return

        # FIX-BUG-4: 验证路径但不使用 UNC 前缀传给 ClamAV（ClamAV 不支持 \\?\ 前缀）
        resolved_target, path_ok = self._normalize_and_verify_path(target)
        if not path_ok:
            result_cb(None, f"扫描目标不存在或无效：{target}")
            return

        # FIX-BUG-4: 移除 UNC 前缀后再传给 ClamAV
        clamav_target = resolved_target
        if IS_WIN and clamav_target.startswith('\\\\?\\'):
            clamav_target = clamav_target[4:]  # 移除 \\?\ 前缀
        elif IS_WIN and clamav_target.startswith('\\\\?\\UNC\\'):
            clamav_target = '\\\\' + clamav_target[8:]  # \\?\UNC\server\share → \\server\share

        self.log(f"开始扫描：{clamav_target}", "info")
        ...
        def _count_files():
            """FIX-BUG-15: 文件计数线程，带超时限制避免阻塞扫描启动。
            大目录下 os.walk 可能耗时数十秒，改为采样估算 + 时间限制。"""
            count = 0
            total_bytes = 0
            deadline = time.time() + 3.0  # 最多允许 3 秒计数
            try:
                ...
                if p.is_file():
                    ...
                    for root, dirs, files in os.walk(resolved_target):
                        ...
                        if time.time() > deadline:
                            # 超时：基于已遍历目录估算总数
                            self.log(f"文件计数超时，基于已遍历部分估算", "dim")
                            break
                        for f in files:
        ...
        try:
            ...
            if exclusions:
                for excl_path in exclusions:
                    ...
                    # FIX-BUG-3: ClamAV PCRE 使用正斜杠，不转义反斜杠
                    excl_posix = excl_resolved.replace(os.sep, '/')  # Windows: C:\... → C:/...
                    excl_escaped = re.escape(excl_posix)
            ...
            for raw_line in proc.stdout:
                if self._scan_cancel.is_set():
                    proc.terminate()
                    self.log("扫描已取消", "warn")
                    # FIX-CUSTOM-SCAN-2: 取消时也调用 result_cb，确保 UI 端 _canceling 被正确重置
                    # 否则 _canceling 永远为 True，导致后续正常扫描完成时 UI 按钮无法恢复
                    result_cb(None, "扫描已取消")
                    return
                ...
                if "FOUND" in line:
                    ...
                    # FIX-BUG: 排除 ClamAV 扫描摘要中的 "Total errors:" 行
                    # "Total errors: N" 是摘要统计，不是真正的扫描错误
                    stripped = line.strip()
                    if stripped.startswith("Total errors"):
                        pass  # 跳过摘要行
                    else:
                        results["errors"] += 1
                        # 记录具体的错误信息以便排查问题
...
class LiangDunApp(tk.Tk):
    def __init__(self):
        ...
        # === 自定义标题栏：Windows 使用无边框窗口，macOS/Linux 使用原生标题栏 ===
        # FIX-BUG-1: 只在 Windows 下移除原生标题栏，macOS/Linux 保留原生标题栏
        if IS_WIN:
        ...
        self._canceling       = False  # FIX-BUG-2: 取消中状态，防止竞态
        ...
        # FIX-THREAD-SAFE: 线程安全的UI更新队列
        self._ui_queue = queue.Queue()
        self._ui_queue_handler_id = None
        
        # FIX-LIFECYCLE: 资源清理标记
        self._is_closing = False
        self._background_tasks = []  # 跟踪所有后台任务
        self._executors = []  # 跟踪所有线程池
        ...
        # FIX-BUG-11: 提前初始化扫描模式，避免定时任务在 _build_scan 前调用时 AttributeError
        self._current_scan_mode = "quick"

        # v7.0.0: HIPS 主动防御
        self._imonitor_mgr = IMonitorManager()
        self._hips_engine = HipsRuleEngine()
        self._hips_running = False
        self._hips_allowed_actions = [
            ("记录", IMonitorAction.Record),
            ("阻止", IMonitorAction.Block),
            ("终止进程", IMonitorAction.TerminateProcess),
            ("挂起进程", IMonitorAction.SuspendProcess),
            ("恢复进程", IMonitorAction.ResumeProcess),
        ]
        ...
        # UI #16: 键盘快捷键
        # FIX-BUG-20/21: 快捷键在输入框时禁用
        def _safe_scan():
            fw = self.focus_get()
            if fw and isinstance(fw, (tk.Entry, tk.Text, tk.scrolledtext.ScrolledText)):
                return
            self._start_scan()
        def _safe_stop():
            fw = self.focus_get()
            ...
            if self._scanning:
                self._stop_scan()
        self.bind("<Control-s>", lambda e: _safe_scan())
        self.bind("<Control-q>", lambda e: self._on_close())
        self.bind("<Escape>",    lambda e: _safe_stop())

        # v6.0.0: 注册退出清理
        atexit.register(self._on_atexit)

    def _on_atexit(self):
        """FIX-LIFECYCLE: 程序退出时的完整清理工作（仅执行非 Tk 操作）"""
        # FIX-BUG-2: 添加幂等性保护，防止重复执行
        ...
        try:
            ...
            logger.debug(f"停止USB监控失败: {e}")
            
        # 3. 取消正在进行的扫描（非 Tk 操作）
        try:
            self.backend.cancel_scan()
        except Exception as e:
            logger.debug(f"取消扫描失败: {e}")
        ...
        # 8. v7.0.0: 停止 HIPS 防御并卸载 iMonitor 驱动
        try:
            if hasattr(self, '_hips_running') and self._hips_running:
                self._imonitor_mgr.stop()
            self._imonitor_mgr.unload()
        except Exception as e:
            logger.debug(f"停止HIPS失败: {e}")
    ...
    def _on_close(self):
        ...
        if self._scanning:
            if not messagebox.askyesno("确认退出",
                    "正在扫描中，退出将中断扫描。\n确定要退出吗？"):
                self._is_closing = False  # 用户取消，恢复状态
                return
            self.backend.cancel_scan()
        if self._updating:
            if not messagebox.askyesno("确认退出",
        ...
        # 取消所有 pending 的 after 回调（使用明确的定时器属性列表，避免遍历 dir(self)）
        # FIX-BUG-3: 移除 _scan_start_time（它是 float 不是 timer ID）
        try:
            for timer_attr in ['_pulse_timer', '_smooth_timer', '_pseudo_timer',
                               '_schedule_timer', '_quar_search_timer', '_ui_queue_handler_id']:
    ...
    def _load_settings(self):
        ...
        # v7.0.0: 加载 HIPS 自启动设置
        hips_autostart = self._settings_data.get("hips_autostart", False)
        if hasattr(self, '_hips_enabled_var'):
            self._hips_enabled_var.set(hips_autostart)

        # FIX-BUG-6: 加载通知和主题设置
        notification = self._settings_data.get("notification", True)
        if hasattr(self, '_notif_var'):
            self._notif_var.set(notification)
        theme = self._settings_data.get("theme", "dark")
    ...
    def _save_settings(self) -> bool:
        ...
        try:
            new_data = {
                ...
                "hips_autostart": self._hips_enabled_var.get() if hasattr(self, '_hips_enabled_var') else False,
                ...
                # FIX-BUG-6: 添加通知和主题设置持久化
                "notification": self._notif_var.get() if hasattr(self, '_notif_var') else True,
    ...
    def _show_eula(self):
        ...
        def _decline():
            ...
            # FIX-UI: 拒绝 EULA 时显示明确提示，而非直接消失
            messagebox.showinfo("退出确认",
                "您已拒绝最终用户许可协议 (EULA)。\n\n"
                "根据协议要求，拒绝后将无法使用本软件。\n"
                "程序将在点击确定后退出。",
                parent=eula_win)
    ...
    def _log(self, msg, tag="info", target=None):
        ...
        FIX-THREAD-SAFE: 使用队列实现线程安全的UI更新
        :param msg: 日志消息
        :param tag: 日志标签
        :param target: 目标日志组件名称 ('scan', 'upd' 或 None 表示所有)
        """
        # FIX-LIFECYCLE: 检查是否正在关闭
        if self._is_closing:
            return
        ...
        if target and target in widget_map:
            # FIX-THREAD-SAFE: 将UI更新放入队列，由主线程处理
            self._queue_ui_action('_append_log_safe', (target, log_line, tag))
        elif target:
            # FIX-BUG-13: target 指定了但不在 widget_map 中，静默丢弃前先警告
            logger.warning(f"[_log] Unknown log target '{target}': {log_line}")
            return
        else:
            # target=None 时写入 Python logger（总是执行）
            logger.info(log_line)
            
    def _append_log_safe(self, target, log_line, tag):
        """FIX-THREAD-SAFE: 在主线程中安全地追加日志"""
        if self._is_closing:
            return
        widget_map = {
            'scan': getattr(self, '_scan_out', None),
        ...
        log_widget = widget_map.get(target)
    ...
    def _queue_ui_action(self, action, args=()):
        """
        FIX-THREAD-SAFE: 将UI操作放入队列，由主线程处理
        :param action: 操作名称或可调用对象
        :param args: 参数元组
        """
        if self._is_closing:
            return
        ...
        # 启动队列处理器（如果尚未启动）
        if self._ui_queue_handler_id is None:
            self._process_ui_queue()
            
    def _process_ui_queue(self):
        """FIX-THREAD-SAFE: 处理UI队列中的待执行操作"""
        if self._is_closing:
            return
            
        processed = 0
        max_per_cycle = 50  # 每次最多处理50个，避免阻塞UI
        while processed < max_per_cycle:
            ...
            try:
                ...
                logger.debug(f"UI队列执行失败: {e}")
            processed += 1
            
        # 如果队列非空，继续调度
        if not self._ui_queue.empty() and not self._is_closing:
            self._ui_queue_handler_id = self.after(10, self._process_ui_queue)
        else:
            self._ui_queue_handler_id = None
    ...
    def _draw_shield(self, status="gray"):
        ...
        """
        # FIX-UI-1: 检查 _home_shield_canvas 是否已创建，防止在 _build_home 之前调用崩溃
        if not hasattr(self, '_home_shield_canvas') or self._home_shield_canvas is None:
            return
    ...
    def _update_dashboard_stats(self):
        ...
        try:
            ...
            week_scans = stats.get("week_scans", 0)
            total_threats = stats.get("total_infected", 0)

            self._dash_week_val.config(text=str(week_scans))
            self._dash_threats_val.config(text=str(total_threats))
            
            # FIX-BUG-17: 同时显示 ClamAV 签名数和 YARA 规则数
            ...
            for info in db_info:
                if info.get("ok"):
                    ...
                    if "条签名" in date_str or "signatures" in date_str.lower():
                        yara_sigs += 1
                    elif "ClamAV" in date_str or "main" in date_str:
                        # 尝试从 date_str 提取签名数
                        import re
                        m = re.search(r'(\d+)', date_str)
                        if m:
                            clamav_sigs = max(clamav_sigs, int(m.group(1)))
            sig_text = f"ClamAV: {clamav_sigs:,} | YARA: {yara_sigs}"
            if hasattr(self, '_dash_sig_val'):
                self._dash_sig_val.config(text=sig_text)

            # FIX-UI-2: 合并签名数更新逻辑，避免重复调用 get_db_info()
            # 更新 _dash_sigs_val（如果存在）
            if hasattr(self, '_dash_sigs_val'):
                total_sig = clamav_sigs + yara_sigs
                if total_sig > 0:
                    self._dash_sigs_val.config(text=f"{total_sig:,}")
                else:
    ...
    def _select_scan_mode(self, mode):
        ...
        # 设置扫描目标
        if mode == "quick":
            # FIX-SECURITY: 快速扫描覆盖病毒高发关键目录，避免假安全感
            if IS_WIN:
                home = str(Path.home())
                ...
                # 病毒高发关键目录：用户目录 + 启动项 + 系统程序目录
                targets = [downloads, desktop, documents]
                if appdata:
                    targets.append(appdata)  # AppData\Roaming
                if localappdata:
                    targets.append(os.path.join(localappdata, "Temp"))
                if temp_dir:
                ...
                startup = os.path.join(appdata, "Microsoft", "Windows", "Start Menu", "Programs", "Startup") if appdata else ""
                if os.path.exists(progfiles):
                    targets.append(progfiles)
                if os.path.exists(progfiles_x86):
                    targets.append(progfiles_x86)
                if startup and os.path.exists(startup):
                    targets.append(startup)
                valid_targets = [t for t in targets if t and os.path.exists(t)]
                if valid_targets:
                    self._scan_target.set(";".join(valid_targets))
                    self._log(f"快速扫描目标: {len(valid_targets)} 个目录", "info")
                else:
                ...
                if os.path.exists("/var/tmp"):
                    targets.append("/var/tmp")
                # 用户级 systemd 启动项
                systemd_user = os.path.join(home, ".config", "systemd", "user")
                if os.path.exists(systemd_user):
                    targets.append(systemd_user)
                valid_targets = [t for t in targets if t and os.path.exists(t)]
                if valid_targets:
                    self._scan_target.set(";".join(valid_targets))
                    self._log(f"快速扫描目标: {len(valid_targets)} 个目录", "info")
    ...
    def _add_scan_target(self):
        ...
        if path:
            # FIX-BUG-5: 正确解析已有目标，避免重复和空字符串
            # FIX-SECURITY: 路径中可能包含 ;（合法字符），使用列表存储而非字符串拼接
            current = self._scan_target.get().strip()
            if current:
                # 尝试从 JSON 列表解析（新格式），失败则回退到 ; 分隔（旧格式）
                try:
            ...
            # FIX-SECURITY: 使用 JSON 列表格式存储，避免 ; 分隔导致路径被截断
            self._scan_target.set(json.dumps(existing, ensure_ascii=False))
            self._log(f"已添加扫描目标: {path} (共 {len(existing)} 个)", "info")
    ...
    def _start_scan(self, mode=None):
        ...
        # v7.0.0: 多目标扫描支持
        # FIX-SECURITY: 兼容 JSON 列表格式和旧版 ; 分隔格式
        multi_targets = []
        if target:
            try:
                parsed = json.loads(target)
                if isinstance(parsed, list):
                    multi_targets = [str(t).strip() for t in parsed if str(t).strip()]
                else:
        ...
        if len(multi_targets) > 1:
            ...
            return
        # FIX-CUSTOM-SCAN-1: 当 JSON 解析得到单元素列表时，用实际路径替换 JSON 字符串
        # 否则 target 仍然是 JSON 格式（如 '["D:\\test"]'），导致后端路径验证失败
        if len(multi_targets) == 1:
    ...
    def _do_update_scan_result(self, results, error):
        ...
        # FIX-BUG-2: 如果正在取消，重置状态并恢复 UI
        if self._canceling:
            self._canceling = False
            self._scanning = False  # FIX-SCAN-LOCK: 确保取消时也重置扫描状态
            # FIX-CUSTOM-SCAN-2: 取消时也恢复 UI 按钮状态，防止按钮卡在 disabled
    ...
    def _stop_scan(self):
        ...
        if not self._scanning:
            return
        # FIX-BUG-2: 使用 _canceling 状态代替立即设置 _scanning = False
        # FIX-SCAN-LOCK: 停止扫描后必须重置 _scanning，否则无法再次扫描
        self._canceling = True
        self._scanning = False  # 立即重置，允许再次启动扫描
    ...
    def _on_usb_insert(self, mount_path):
        ...
        # FIX-BUG-23: 询问用户是否扫描，不自动扫描
        if messagebox.askyesno("USB 设备", f"检测到 USB 设备 {mount_path}\n是否立即扫描?"):
            try:
                self._scan_custom_target(str(mount_path))
            except Exception as e:
                self._log(f"USB 设备扫描失败: {e}", "warn")
        else:
            self._log(f"USB 扫描已取消", "info")
    ...
    def _scan_multi_targets(self, targets):
        ...
        def _run_scan():
            ...
            def scan_one(t):
                """扫描单个目标"""
                if self.backend._scan_cancel.is_set():
                    return
                ...
                try:
                    ...
                    # FIX-多目标实时显示: 实现进度回调，实时显示正在扫描的文件
                    def progress_cb(pct, current_file=None, scanned=None, total=None):
                        if current_file:
                            self.after(0, lambda f=current_file:
                                self._current_scan_file.set(f"正在扫描: {f}"))
                        if pct is not None and pct >= 0:
                            # 综合进度 = 已完成目标占比 + 当前目标内进度占比
                            with lock:
                                base_pct = (completed[0] / len(targets)) * 100
                                target_pct = (1 / len(targets)) * pct
                                combined = min(int(base_pct + target_pct), 99)
                            self.after(0, lambda p=combined: self._scan_pb.config(value=p))
                            self.after(0, lambda p=combined: self._scan_pct_label.config(text=f"{p}%"))
                ...
                with lock:
                    ...
                    self.after(0, lambda p=pct: self._scan_pct_label.config(text=f"{p}%"))
                    self.after(0, lambda c=completed[0], total=len(targets):
                        self._current_scan_file.set(f"已完成 {c}/{total} 个目标"))

            from concurrent.futures import ThreadPoolExecutor, as_completed
            # FIX-RACE: backend.scan() 内部使用 _scan_lock 保证串行执行，
            # max_workers=1 配合锁确保多目标串行扫描，不会阻塞 UI。
    ...
    def _do_pulse_card(self, parent, color, step, total_steps):
        """执行卡片脉冲动画"""
        # FIX-DESTROY: 动画期间窗口可能被销毁，全程检查生命周期
        try:
    ...
    def _build_quarantine(self, parent):
        ...
        # FIX-UI-5: 使用标记变量区分 placeholder 和用户输入
        _placeholder_text = "搜索隔离文件..."
        _is_placeholder = [True]  # 用列表包装以便在闭包中修改

        search_entry.insert(0, _placeholder_text)
        search_entry.config(fg=C["dim"])  # placeholder 使用暗淡颜色

        def _on_focus_in(e):
            if _is_placeholder[0]:
                search_entry.delete(0, "end")
                search_entry.config(fg=C["text"])
                _is_placeholder[0] = False

        def _on_focus_out(e):
            if not search_entry.get().strip():
                search_entry.insert(0, _placeholder_text)
                search_entry.config(fg=C["dim"])
                _is_placeholder[0] = True
    ...
    def _do_quar_search(self):
        ...
        # FIX-UI-5: 使用标记变量判断 placeholder，而非字符串比较
        if keyword == "搜索隔离文件..." or not keyword:
            keyword = ""
    ...
    def _build_settings(self, parent):
        ...
        # FIX-SECURITY: GPU 加速功能为虚假功能，ClamAV 不支持 GPU 加速
        # 保留变量兼容旧配置，但 UI 中移除开关和误导性描述
        # 仅显示 GPU 检测信息（无开关）
        gpu_info = get_gpu_info()
        ...
        tk.Label(gpu_status_frame, text="GPU 状态",
                 bg=C["card"], fg=C["text"],
                 font=(FONT_FAMILY, 10), anchor="w").pack(side="left")
        ...
        # FIX-SECURITY: 强制禁用 GPU 加速变量，不显示开关
        self._gpu_accel_var.set(False)

        gpu_desc = tk.Frame(sf, bg=C["bg"])
        gpu_desc.pack(fill="x", pady=(0, 12))
        gpu_desc_text = "当前版本使用 CPU 扫描。GPU 加速暂不支持（ClamAV 引擎限制）。"
        ...
        # ── HIPS 主动防御 ──
        self._settings_section(sf, "HIPS 主动防御")

        hips_rt_frame = tk.Frame(sf, bg=C["card"],
                                  highlightthickness=1,
                                  highlightbackground=C["border"])
        hips_rt_frame.pack(fill="x", pady=(0, 12), ipady=8)

        self._hips_enabled_var = tk.BooleanVar(value=False)
        tk.Label(hips_rt_frame, text="启用 HIPS 主动防御 (开机自启)",
                 bg=C["card"], fg=C["text"],
                 font=(FONT_FAMILY, 10), anchor="w").pack(side="left", padx=16)

        self._create_styled_checkbutton(hips_rt_frame, variable=self._hips_enabled_var,
                                        command=self._toggle_hips_autostart).pack(
            side="right", padx=16)

        hips_rt_desc = tk.Frame(sf, bg=C["bg"])
        hips_rt_desc.pack(fill="x", pady=(0, 12))
        tk.Label(hips_rt_desc,
                 text="启用后将加载 iMonitor 内核驱动，拦截恶意进程创建、文件篡改、注册表修改等行为。需要管理员权限。",
                 bg=C["bg"], fg=C["dim"],
                 font=(FONT_FAMILY, 9), anchor="w", wraplength=600,
                 justify="left").pack(fill="x")
    ...
    def _save_and_apply_settings(self):
        """保存并应用设置 - FIX-ROLLBACK: 自启动失败时回滚 JSON 和 UI 状态"""
        # 记录自启动变更前的状态，用于回滚
        prev_autostart_ui = self._autostart.get()
        ...
        if autostart_ok:
            ...
            # FIX-ROLLBACK: 自启动失败，回滚 JSON 中的 autostart 字段和 UI checkbox
            self._log(f"自启动设置失败: {autostart_msg}", "warn")
            # FIX-BUG: 回滚 JSON 设置文件中的 autostart 字段为变更前的值
            try:
                if self._settings_file.exists():
                    data = json.loads(self._settings_file.read_text(encoding="utf-8"))
                    # FIX-SECURITY: prev_autostart_ui 是变更前的值，回滚时应恢复为该值
                    data["autostart"] = prev_autostart_ui
                    self._settings_file.write_text(
                        json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")
                    self._settings_data["autostart"] = prev_autostart_ui
            except Exception as e:
                self._log(f"回滚设置文件失败: {e}", "warn")
            # 回滚 UI checkbox 状态
            self._autostart.set(prev_autostart_ui)
            self._settings_dirty = False
            try:
                if hasattr(self, '_settings_hint_lbl') and self._settings_hint_lbl.winfo_exists():
                    self._settings_hint_lbl.config(text="")
                ...
                pass
            self._toast.show("自启动设置失败",
                             f"自启动设置失败: {autostart_msg}，其他设置已保存",
                             icon_type="warning")
    ...
    def _switch_tab(self, idx):
        ...
        if self._settings_dirty and idx != 6:
            if not messagebox.askyesno("未保存的更改",
                    "您在「系统设置」中有未保存的更改。\n"
                    "如果离开此页面，更改将丢失。\n\n"
                    "确定要放弃更改并离开吗？"):
    ...
    def _fade_in_page(self, idx, step=0, total_steps=3):
        """页面切换 fade-in 效果"""
        # FIX-UI-4: 减少 total_steps 从 6 到 3，降低递归次数提升性能
        # FIX-DESTROY: 动画期间窗口可能被销毁，全程检查生命周期
        try:
            if not self.winfo_exists():
                return
        ...
        try:
            ...
            # FIX-UI-4: 限制递归深度，避免复杂页面性能问题
            def _recursive_update_bg(widget, color, depth=0):
                """递归更新子控件背景色（限制深度）"""
                if depth > 5:  # 最多递归 5 层
                    return
                try:
                ...
                try:
                    for child in widget.winfo_children():
                        _recursive_update_bg(child, color, depth + 1)
                except Exception:
                    pass
    ...
    # ═══════════════════════════════════════════════════════════════
    #  HIPS 主动防御页面（v7.0.0 新增）
    # ═══════════════════════════════════════════════════════════════
    def _build_hips(self, parent):
        page = tk.Frame(parent, bg=C["bg"])
        page.pack(fill="both", expand=True)
        ...
        tk.Label(header, text="\U0001f6e1  HIPS 主动防御", bg=C["bg"], fg=C["white"],
                 font=(FONT_FAMILY_BOLD, 14, "bold")).pack(side="left")

        # 启动/停止按钮
        self._hips_toggle_btn = tk.Button(
            header, text="启动防御", bg=C["accent"], fg=C["white"],
            font=(FONT_FAMILY, 10, "bold"), relief="flat", bd=0, padx=16, pady=6,
            cursor="hand2", command=self._toggle_hips,
            activebackground=_lighten(C["accent"], 0.15), activeforeground=C["white"]
        )
        self._hips_toggle_btn.pack(side="right")

        # 状态栏
        status_frame = tk.Frame(page, bg=C["card"],
                                highlightthickness=1,
                                highlightbackground=C["border"])
        status_frame.pack(fill="x", padx=30, pady=(0, 16), ipady=12)

        self._hips_status_lbl = tk.Label(status_frame, text="防御状态: 未启动",
                                         bg=C["card"], fg=C["dim"],
                                         font=(FONT_FAMILY, 11))
        self._hips_status_lbl.pack(side="left", padx=20, pady=8)

        self._hips_stats_lbl = tk.Label(status_frame, text="已加载规则: 0 | 拦截事件: 0",
                                        bg=C["card"], fg=C["dim"],
                                        font=(FONT_FAMILY, 9))
        self._hips_stats_lbl.pack(side="right", padx=20, pady=8)

        # 规则列表区域
        rules_header = tk.Frame(page, bg=C["bg"])
        rules_header.pack(fill="x", padx=30, pady=(0, 4))

        tk.Label(rules_header, text="防御规则", bg=C["bg"], fg=C["text"],
                 font=(FONT_FAMILY_BOLD, 11, "bold")).pack(side="left")

        # 添加规则按钮
        self._btn(rules_header, "+ 添加规则", self._hips_add_rule,
                  color=C["accent"]).pack(side="right")

        # 规则 Treeview
        tree_frame = tk.Frame(page, bg=C["card"],
                              highlightthickness=1,
                              highlightbackground=C["border"])
        tree_frame.pack(fill="both", expand=True, padx=30, pady=(0, 16))

        columns = ("enabled", "name", "event", "action", "builtin")
        self._hips_tree = ttk.Treeview(tree_frame, columns=columns,
                                       show="headings", height=12)
        self._hips_tree.heading("enabled", text="启用")
        self._hips_tree.heading("name", text="规则名称")
        self._hips_tree.heading("event", text="事件类型")
        self._hips_tree.heading("action", text="动作")
        self._hips_tree.heading("builtin", text="来源")

        self._hips_tree.column("enabled", width=50, anchor="center")
        self._hips_tree.column("name", width=220, anchor="w")
        self._hips_tree.column("event", width=160, anchor="w")
        self._hips_tree.column("action", width=80, anchor="center")
        self._hips_tree.column("builtin", width=60, anchor="center")

        # Treeview 样式
        # FIX-UI-7: macOS 上 Treeview 背景色支持有限，使用原生样式
        style = ttk.Style()
        if sys.platform == "darwin":
            # macOS: 使用原生样式，只设置字体
            style.configure("Treeview", font=(FONT_FAMILY, 9), borderwidth=0)
            style.configure("Treeview.Heading", font=(FONT_FAMILY_BOLD, 9),
                            borderwidth=0, relief="flat")
        else:
            style.configure("Treeview", background=C["card"], foreground=C["text"],
                            fieldbackground=C["card"], borderwidth=0, font=(FONT_FAMILY, 9))
            style.configure("Treeview.Heading", background=C["panel"], foreground=C["white"],
                            font=(FONT_FAMILY_BOLD, 9), borderwidth=0, relief="flat")
            style.map("Treeview.Heading", background=[("active", C["accent"])])

        scrollbar = tk.Scrollbar(tree_frame, command=self._hips_tree.yview,
                                 bg=C["border"], troughcolor=C["bg"])
        self._hips_tree.configure(yscrollcommand=scrollbar.set)
        self._hips_tree.pack(side="left", fill="both", expand=True)
        scrollbar.pack(side="right", fill="y")

        # 双击切换启用状态
        self._hips_tree.bind("<Double-1>", self._hips_toggle_rule)

        # 右键菜单
        self._hips_context_menu = tk.Menu(self._hips_tree, tearoff=0,
                                          bg=C["card"], fg=C["text"],
                                          font=(FONT_FAMILY, 9))
        self._hips_context_menu.add_command(label="启用/禁用", command=self._hips_toggle_rule)
        self._hips_context_menu.add_command(label="删除规则", command=self._hips_delete_rule)
        self._hips_tree.bind("<Button-3>", self._hips_show_context_menu)

        return page

    def _refresh_hips(self):
        """刷新 HIPS 页面状态"""
        if not hasattr(self, '_hips_engine'):
            return
        try:
            summary = self._hips_engine.get_summary()
            stats = self._hips_engine.get_stats()

            # FIX-SECURITY: 显示真实运行状态（是否真正加载了内核驱动）
            is_real = (self._imonitor_mgr._loaded and self._imonitor_mgr._dll is not None)
            if self._hips_running:
                if is_real:
                    status_text = "防御状态: 运行中（内核级）"
                    status_color = C["green"]
                else:
                    status_text = "防御状态: 运行中（模拟模式）"
                    status_color = C["warn"]
            else:
                status_text = "防御状态: 未启动"
                status_color = C["dim"]
            self._hips_status_lbl.config(text=status_text, fg=status_color)

            total_blocks = (stats.get("process_blocked", 0) +
                          stats.get("file_blocked", 0) +
                          stats.get("registry_blocked", 0) +
                          stats.get("network_blocked", 0))
            self._hips_stats_lbl.config(
                text=f"已加载规则: {summary.get('enabled', 0)}/{summary.get('total', 0)} | 拦截事件: {total_blocks}")

            # 刷新规则列表
            self._hips_tree.delete(*self._hips_tree.get_children())
            action_names = {a: n for n, a in self._hips_allowed_actions}
            for rule in self._hips_engine.get_all_rules():
                enabled = "✔" if rule.get("enabled", True) else "✘"
                action_name = action_names.get(rule.get("action", 0), "未知")
                source = "内置" if rule.get("builtin", False) else "自定义"
                event_type = rule.get("event", "")
                self._hips_tree.insert("", "end",
                    values=(enabled, rule.get("name", rule.get("_id", "")),
                           event_type, action_name, source))
        except Exception as e:
            logger.warning("HIPS 页面刷新失败: %s", e)

    def _toggle_hips(self):
        """启动/停止 HIPS 防御"""
        if self._hips_running:
            # 停止
            self._hips_engine.stop() if hasattr(self._hips_engine, 'stop') else None
            self._imonitor_mgr.stop()
            self._hips_running = False
            self._hips_toggle_btn.config(text="启动防御", bg=C["accent"])
            self._log("HIPS 主动防御已停止", "info")
        else:
            # 启动
            self._hips_engine.load_all()
            im_ok = self._imonitor_mgr.load()
            if im_ok:
                self._imonitor_mgr.start()
            self._hips_running = True

            # FIX-SECURITY: 检测 HIPS 是否为真实运行（DLL 是否真正加载成功）
            is_real = im_ok and self._imonitor_mgr._loaded and self._imonitor_mgr._dll is not None
            if is_real:
                self._hips_toggle_btn.config(text="停止防御", bg=C["danger"])
                self._log("HIPS 主动防御已启动（内核驱动已加载）", "success")
            else:
                # 空壳状态：DLL 未真实加载，规则匹配只是演示
                self._hips_toggle_btn.config(text="停止防御 [模拟模式]", bg=C["warn"])
                self._log("HIPS 主动防御已启动（模拟模式：内核驱动未加载，规则匹配仅作演示）", "warn")
                self._toast.show("HIPS 模拟模式",
                    "iMonitor 内核驱动未加载，HIPS 处于模拟模式。\n"
                    "规则匹配功能可用，但内核级拦截不会生效。\n"
                    "请以管理员身份运行并安装驱动以获得完整防护。",
                    icon_type="warning")

            # 应用规则到 iMonitor
            rules_json = self._hips_engine.export_rules_json()
            self._imonitor_mgr.set_msg_rules(rules_json)

        self._refresh_hips()
        # FIX-PERSIST: 立即保存设置
        self._save_settings()

    def _toggle_hips_autostart(self):
        """设置页 HIPS 自启开关回调"""
        # FIX-PERSIST: 保存 HIPS 自启动设置
        self._mark_settings_dirty()
        if self._hips_enabled_var.get():
            self._log("HIPS 主动防御自启动已启用", "info")
            # 尝试立即启动 HIPS
            if not self._hips_running:
                self._toggle_hips()
        else:
            self._log("HIPS 主动防御自启动已禁用", "info")

    def _hips_toggle_rule(self, event=None):
        """切换规则启用/禁用状态"""
        selection = self._hips_tree.selection()
        if not selection:
            return
        item = selection[0]
        values = self._hips_tree.item(item, "values")
        if not values:
            return
        rule_name = values[1]
        all_rules = self._hips_engine.get_all_rules()
        for rule in all_rules:
            if rule.get("name", rule.get("_id", "")) == rule_name:
                self._hips_engine.toggle_rule(rule.get("_id", ""))
                break
        self._refresh_hips()

    def _hips_delete_rule(self, event=None):
        """删除自定义规则"""
        selection = self._hips_tree.selection()
        if not selection:
            return
        ...
        # 内置规则不允许删除
        if values[4] == "内置":
            self._toast.show("操作拒绝", "内置规则不可删除", icon_type="warning")
            return
        rule_name = values[1]
        all_rules = self._hips_engine.get_all_rules()
        for rule in all_rules:
            if rule.get("name", rule.get("_id", "")) == rule_name and not rule.get("builtin", False):
                if messagebox.askyesno("确认删除", f"确定要删除规则「{rule_name}」吗？"):
                    self._hips_engine.delete_custom_rule(rule.get("_id", ""))
                break
        self._refresh_hips()

    def _hips_show_context_menu(self, event):
        """显示右键菜单"""
        self._hips_context_menu.post(event.x_root, event.y_root)

    def _hips_add_rule(self):
        """弹出添加规则对话框"""
        add_win = tk.Toplevel(self)
        add_win.title("添加 HIPS 规则")
        add_win.geometry("520x420")
        add_win.resizable(False, False)
        add_win.transient(self)
        add_win.grab_set()
        add_win.configure(bg=C["bg"])

        tk.Label(add_win, text="添加 HIPS 主动防御规则", bg=C["bg"], fg=C["white"],
                 font=(FONT_FAMILY_BOLD, 12, "bold")).pack(anchor="w", padx=20, pady=(16, 8))

        form = tk.Frame(add_win, bg=C["bg"])
        form.pack(fill="both", expand=True, padx=20, pady=8)

        # 规则名称
        tk.Label(form, text="规则名称", bg=C["bg"], fg=C["text"],
                 font=(FONT_FAMILY, 10), anchor="w").pack(fill="x", pady=(8, 2))
        name_var = tk.StringVar()
        tk.Entry(form, textvariable=name_var, bg=C["card"], fg=C["text"],
                 font=(FONT_FAMILY, 10), relief="flat", bd=0,
                 insertbackground=C["accent"]).pack(fill="x", ipady=4)

        # 事件类型
        tk.Label(form, text="事件类型", bg=C["bg"], fg=C["text"],
                 font=(FONT_FAMILY, 10), anchor="w").pack(fill="x", pady=(8, 2))
        event_var = tk.StringVar()
        event_names = [e[0] for e in HipsRuleEngine.EVENT_TYPES]
        event_var.set(event_names[0])
        tk.OptionMenu(form, event_var, *event_names).pack(fill="x")
        # FIX-HIPS-7: OptionMenu 深色主题修复 - 遍历所有子控件包括 Menubutton
        for child in form.winfo_children():
            if isinstance(child, tk.OptionMenu):
                child.config(bg=C["card"], fg=C["text"],
                ...
                # 修复内部 Menubutton 样式
                try:
                    child["menu"].config(bg=C["card"], fg=C["text"],
                                         activebackground=C["accent"],
                                         activeforeground=C["white"],
                                         selectcolor=C["card"])
                except Exception:
        ...
        # 匹配路径
        tk.Label(form, text="匹配路径 (留空匹配所有)", bg=C["bg"], fg=C["text"],
                 font=(FONT_FAMILY, 10), anchor="w").pack(fill="x", pady=(8, 2))
        path_var = tk.StringVar(value="*")
        tk.Entry(form, textvariable=path_var, bg=C["card"], fg=C["text"],
                 font=(FONT_FAMILY, 10), relief="flat", bd=0,
                 insertbackground=C["accent"]).pack(fill="x", ipady=4)

        # 动作
        tk.Label(form, text="匹配后的动作", bg=C["bg"], fg=C["text"],
                 font=(FONT_FAMILY, 10), anchor="w").pack(fill="x", pady=(8, 2))
        action_var = tk.StringVar()
        action_names = [a[0] for a in self._hips_allowed_actions]
        action_var.set(action_names[0])
        tk.OptionMenu(form, action_var, *action_names).pack(fill="x")
        # FIX-HIPS-7: OptionMenu 深色主题修复 - 遍历所有子控件包括 Menubutton
        for child in form.winfo_children():
            if isinstance(child, tk.OptionMenu):
                child.config(bg=C["card"], fg=C["text"],
        ...
        # 确认按钮
        btn_frame = tk.Frame(add_win, bg=C["bg"])
        btn_frame.pack(fill="x", padx=20, pady=16)

        def _confirm_add():
            name = name_var.get().strip()
            if not name:
                self._toast.show("错误", "请输入规则名称", icon_type="warning")
                return
            event_name = event_var.get()
            action_name = action_var.get()

            # 映射事件类型
            event_id = HipsRuleEngine.EVENT_TYPE_MAP.get(event_name, "")
            # 映射动作
            action_map = {n: a for n, a in self._hips_allowed_actions}
            action_id = action_map.get(action_name, IMonitorAction.Record)
            ...
            rule = {
                ...
                "matcher": {
                    "and": [
                        {"match": ["Path", path_var.get()]}
                    ]
                }
            }

            if self._hips_engine.add_custom_rule(rule):
                self._toast.show("成功", f"规则「{name}」已添加", icon_type="success")
                self._refresh_hips()
                add_win.destroy()
            else:
                self._toast.show("失败", "规则添加失败", icon_type="error")

        self._btn(btn_frame, "确认添加", _confirm_add, color=C["accent"]).pack(side="left")
        self._btn(btn_frame, "取消", add_win.destroy, color=C["dim"]).pack(side="left", padx=8)
    ...
    def _build_ui(self):
        ...
        self._page_hips       = self._build_hips(self._nb)
        self._page_settings   = self._build_settings(self._nb)
        self._page_about      = self._build_about(self._nb)

        # FIX-BUG-5: _build_settings 后才创建 _hips_enabled_var，需要重新加载 HIPS 设置
        hips_autostart = self._settings_data.get("hips_autostart", False)
        if hasattr(self, '_hips_enabled_var'):
            self._hips_enabled_var.set(hips_autostart)
        ...
        # FIX-BUG-25: Tab 切换时重置键盘焦点
        def _on_tab_change(event):
            if hasattr(self, '_page_scan'):
                for w in self._page_scan.winfo_children():
                    w.focus_set()
                    break
        self._nb.bind("<<NotebookTabChanged>>", _on_tab_change)
        
        self._nb.add_tab("隔离箱", self._page_quarantine)
        self._nb.add_tab("HIPS",  self._page_hips)
    ...
    def _init_check(self):
        ...
        # v7.0.0: 检查 iMonitor HIPS 内核驱动
        self._log("检查 HIPS 内核驱动...", "info")
        hips_available = self._imonitor_mgr.load()
        if hips_available:
            self._log("HIPS: iMonitor 内核驱动已就绪", "success")
            self._hips_engine.load_all()
            summary = self._hips_engine.get_summary()
            self._log(f"HIPS: 已加载 {summary.get('enabled', 0)}/{summary.get('total', 0)} 条规则", "info")

            # FIX-HIPS-PERSIST: 根据 hips_autostart 设置自动启动 HIPS 防护
            hips_autostart = self._settings_data.get("hips_autostart", False)
            if hips_autostart and not self._hips_running:
                self._log("HIPS: 自动启动主动防御（自启动设置）", "info")
                try:
                    self._imonitor_mgr.start()
                    self._hips_running = True
                    rules_json = self._hips_engine.export_rules_json()
                    ...
                    # 更新 HIPS 页面按钮状态
                    try:
                        if hasattr(self, '_hips_toggle_btn') and self._hips_toggle_btn.winfo_exists():
                            self._hips_toggle_btn.config(text="停止防御", bg=C["danger"])
                    except Exception:
                        pass
                    self._refresh_hips()
                    self._log("HIPS: 主动防御已自动启动", "success")
                except Exception as e:
                    self._log(f"HIPS: 自动启动失败: {e}", "warn")
        else:
            self._log("HIPS: iMonitor 驱动不可用（需要管理员权限安装驱动）", "warn")
    ...
    def _on_gpu_accel_toggle(self):
        """OPT-GPU: GPU 加速开关切换
        FIX-BUG-9: ClamAV 不支持 GPU 加速，此功能为虚假功能，已禁用
        保留设置变量以兼容旧配置，但实际不执行任何 GPU 相关操作
        """
        # FIX-BUG-9: ClamAV 引擎本身不支持 GPU 加速
        # 此设置仅保留配置兼容性，实际扫描仍使用 CPU
        self._gpu_accel_var.set(False)
        self._toast.show("GPU 加速", "ClamAV 引擎暂不支持 GPU 加速，将使用 CPU 扫描",
                        icon_type="info")
        self._log("GPU 加速不可用: ClamAV 引擎不支持 GPU 加速", "info")
        self._mark_settings_dirty()
...
# ═══════════════════════════════════════════════════════════════
#  iSDK 内核驱动集成（v7.0.0 新增完整封装）
#  提供：强制结束进程 / 强制删除文件 / 内核级权限提升
# ═══════════════════════════════════════════════════════════════

# 全局 iSDK 状态
_isdk_dll_handle = None
_isdk_initialized = False
_isdk_available   = False


def ensure_isdk_available():
    """
    加载并初始化 iSDK 内核驱动。
    成功加载后，进程获得内核级操作能力（强制结束进程、强制删除文件等）。
    :return: True 表示 iSDK 已就绪
    """
    global _isdk_dll_handle, _isdk_initialized, _isdk_available

    if _isdk_initialized:
        return _isdk_available

    _isdk_initialized = True

    if not IS_WIN:
        logger.info("iSDK 仅支持 Windows，已跳过")
        return False

    if not ISWORD_DLL.exists():
        logger.warning("iSDK DLL 缺失: %s", ISWORD_DLL.name)
        return False

    if not ISWORD_SYS.exists():
        logger.warning("iSDK 驱动文件缺失: %s", ISWORD_SYS.name)
        return False

    try:
        # 添加 DLL 搜索路径（Windows 7+ 支持）
        if hasattr(os, "add_dll_directory"):
            os.add_dll_directory(str(ISDK_BIN_DIR))

        # 加载 iSDK DLL
        _isdk_dll_handle = ctypes.WinDLL(str(ISWORD_DLL))
        logger.info("iSDK DLL 已加载: %s", ISWORD_DLL.name)

        # 调用 DllRegisterServer 完成内核驱动初始化
        try:
            reg_result = _isdk_dll_handle.DllRegisterServer()
            if reg_result == 0:
                logger.info("iSDK 内核驱动初始化成功 (DllRegisterServer OK)")
            else:
                logger.warning("iSDK DllRegisterServer 返回: 0x%08X", reg_result & 0xFFFFFFFF)
        except Exception as e:
            logger.warning("iSDK DllRegisterServer 调用失败: %s", e)

        _isdk_available = True
        return True

    except Exception as e:
        logger.warning("iSDK 加载失败: %s", e)
        _isdk_dll_handle = None
        return False


def isdk_is_available():
    """检查 iSDK 内核驱动是否已就绪"""
    return _isdk_available and _isdk_dll_handle is not None


def isdk_force_kill_process(pid: int) -> bool:
    """
    使用 iSDK 内核驱动强制终止进程（包括受保护的进程）。
    先尝试标准终止，失败后使用内核级强制终止。
    :param pid: 目标进程 ID
    :return: True 表示进程已终止
    """
    if not IS_WIN:
    ...
    try:
        ...
        if hProcess:
            # 尝试标准终止
            kernel32.TerminateProcess(hProcess, 0xFFFFFFFF)
            kernel32.WaitForSingleObject(hProcess, 3000)
            kernel32.CloseHandle(hProcess)

            # 验证进程是否已退出
            check_h = kernel32.OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, False, pid)
            if check_h:
                exit_code = ctypes.c_ulong()
                kernel32.GetExitCodeProcess(check_h, ctypes.byref(exit_code))
                kernel32.CloseHandle(check_h)
                if exit_code.value != 259:  # STILL_ACTIVE
                    logger.info("iSDK: 进程 PID=%d 已标准终止", pid)
                    return True
            else:
                # 进程不存在 = 已终止
                logger.info("iSDK: 进程 PID=%d 已终止", pid)
                return True
    except Exception as e:
        logger.debug("iSDK: 标准终止失败 PID=%d: %s", pid, e)

    # 方法 2: 内核级强制终止（使用 iSDK 驱动能力）
    if isdk_is_available():
        try:
            # 通过已加载的 iSDK DLL，使用 Windows API 调用
            # 驱动加载后，OpenProcess + TerminateProcess 可穿透保护
            kernel32 = ctypes.windll.kernel32
            PROCESS_ALL_ACCESS = 0x001F0FFF
            hProcess = kernel32.OpenProcess(PROCESS_ALL_ACCESS, False, pid)
            if not hProcess:
                logger.warning("iSDK: 无法打开进程 PID=%d (错误码: %d)", pid,
                             ctypes.get_last_error())
                return False

            # 强制终止
            result = kernel32.TerminateProcess(hProcess, 0xFFFFFFFF)
            kernel32.WaitForSingleObject(hProcess, 5000)
            kernel32.CloseHandle(hProcess)

            if result:
                logger.info("iSDK: 进程 PID=%d 已强制终止", pid)
                return True
            else:
                logger.warning("iSDK: 强制终止失败 PID=%d (错误码: %d)", pid,
                             ctypes.get_last_error())
                return False
        except Exception as e:
            logger.warning("iSDK: 强制终止异常 PID=%d: %s", pid, e)
            return False
    else:
        logger.warning("iSDK: 不可用，无法强制终止 PID=%d", pid)
        return False
...
def isdk_suspend_process(pid: int) -> bool:
    """
    使用 Windows API 挂起进程（暂停所有线程）。
    :param pid: 目标进程 ID
    :return: True 表示进程已挂起
    """
    if not IS_WIN:
    ...
    try:
        ...
        if hSnapshot == -1:
            logger.warning("iSDK: 无法创建线程快照 PID=%d", pid)
            return False
        ...
        if kernel32.Thread32First(hSnapshot, ctypes.byref(te32)):
            while True:
                if te32.th32OwnerProcessID == pid:
                    hThread = kernel32.OpenThread(0x0002, False, te32.th32ThreadID)  # THREAD_SUSPEND_RESUME
                    if hThread:
        ...
        if suspended_count > 0:
            logger.info("iSDK: 进程 PID=%d 已挂起 (挂起了 %d 个线程)", pid, suspended_count)
            return True
        else:
            logger.warning("iSDK: 未找到进程 PID=%d 的线程", pid)
            return False
    except Exception as e:
        logger.warning("iSDK: 挂起进程异常 PID=%d: %s", pid, e)
        return False


def isdk_resume_process(pid: int) -> bool:
    """
    使用 Windows API 恢复进程（恢复所有线程）。
    :param pid: 目标进程 ID
    :return: True 表示进程已恢复
    """
    if not IS_WIN:
    ...
    try:
        ...
        if resumed_count > 0:
            logger.info("iSDK: 进程 PID=%d 已恢复 (恢复了 %d 个线程)", pid, resumed_count)
            return True
        else:
            logger.warning("iSDK: 未找到进程 PID=%d 的线程", pid)
            return False
    except Exception as e:
        logger.warning("iSDK: 恢复进程异常 PID=%d: %s", pid, e)
        return False


def isdk_force_delete_file(file_path) -> bool:
    """
    使用 iSDK 内核驱动强制删除文件（包括被占用/锁定的文件）。
    先尝试标准删除，失败后使用内核级强制删除。
    :param file_path: 文件路径（str 或 Path 对象）
    :return: True 表示文件已删除
    """
    if not IS_WIN:
        return False
    ...
    if not file_path.exists():
        logger.debug("iSDK: 文件不存在，无需删除: %s", file_path)
        return True

    # 方法 1: 标准删除（先尝试正常途径）
    try:
        # 移除只读属性
        file_path.chmod(file_path.stat().st_mode | stat.S_IWUSR | stat.S_IWGRP)
        file_path.unlink()
        if not file_path.exists():
            logger.info("iSDK: 文件已标准删除: %s", file_path.name)
            return True
    except PermissionError:
        pass
    except Exception as e:
        logger.debug("iSDK: 标准删除失败: %s", e)

    if not isdk_is_available():
        logger.warning("iSDK: 不可用，无法强制删除: %s", file_path.name)
        return False

    # 方法 2: 内核级强制删除
    try:
        # 方案 A: 关闭所有文件句柄后删除（内核驱动能力）
        kernel32 = ctypes.windll.kernel32

        # 先将文件移动到临时位置（绕过占用锁）
        import tempfile
        temp_dir = Path(tempfile.gettempdir())
        temp_name = f"~isdk_del_{int(time.time()*1000)}_{file_path.name}"
        temp_path = temp_dir / temp_name

        # 尝试 MoveFileEx + 延迟删除标记
        MOVEFILE_DELAY_UNTIL_REBOOT = 0x00000004
        MOVEFILE_REPLACE_EXISTING = 0x00000001

        # 先尝试直接 MoveFileEx 标记为重启后删除
        result = kernel32.MoveFileExW(
            str(file_path), None,
            MOVEFILE_DELAY_UNTIL_REBOOT
        )
        if result:
            logger.info("iSDK: 文件已标记为重启后删除: %s", file_path.name)
            return True

        # 方案 B: 尝试移动后删除
        try:
            result = kernel32.MoveFileExW(
                str(file_path), str(temp_path),
                MOVEFILE_REPLACE_EXISTING
            )
            if result and temp_path.exists():
                temp_path.unlink()
                logger.info("iSDK: 文件已移动删除: %s", file_path.name)
                return True
        except Exception:
            pass

        # 方案 C: 直接调用 DeleteFileW（内核驱动加持）
        result = kernel32.DeleteFileW(str(file_path))
        if result:
            logger.info("iSDK: 文件已内核删除: %s", file_path.name)
            return True

        # 方案 D: 最后兜底 - 使用 cmd 的 del 命令（有时能绕过占用）
        try:
            subprocess.run(
            ...
            if not file_path.exists():
                logger.info("iSDK: 文件已 cmd 删除: %s", file_path.name)
                return True
        except Exception:
            pass

        logger.warning("iSDK: 所有删除方法均失败: %s", file_path.name)
        return False

    except Exception as e:
        logger.warning("iSDK: 强制删除异常 %s: %s", file_path.name, e)
        return False


def isdk_force_delete_registry_key(reg_path: str) -> bool:
    """
    使用 iSDK 内核驱动强制删除注册表键（含所有子键和值）。
    :param reg_path: 注册表路径，如 HKEY_LOCAL_MACHINE\\Software\\MalwareKey
    :return: True 表示删除成功
    """
    if not IS_WIN:
        return False
    try:
        ...
        if root_key is None:
            logger.warning("iSDK: 无法识别的注册表根键: %s", root_name)
            return False

        sub_key = parts[1] if len(parts) > 1 else ""

        def _delete_key_recursive(key_handle, subkey_name):
            """递归删除注册表子键"""
            try:
                with winreg.OpenKey(key_handle, subkey_name, 0,
                                    winreg.KEY_ALL_ACCESS) as sub_handle:
                    # 先删除所有子键
                    i = 0
                    while True:
                        try:
                            child_name = winreg.EnumKey(sub_handle, 0)
                            _delete_key_recursive(sub_handle, child_name)
                        except OSError:
                            break
                        i += 1
                        if i > 10000:
                            break
                # 删除本键
                winreg.DeleteKey(key_handle, subkey_name)
                return True
            except FileNotFoundError:
                return True  # 已不存在
            except Exception as e:
                logger.debug("iSDK: 删除注册表键失败 %s: %s", subkey_name, e)
                return False

        result = _delete_key_recursive(root_key, sub_key)
        if result:
            logger.info("iSDK: 注册表键已删除: %s", reg_path)
        return result
    except Exception as e:
        logger.warning("iSDK: 删除注册表键异常 %s: %s", reg_path, e)
        if isdk_is_available():
            # 内核驱动加持下重试
            try:
                import winreg
                ...
                parts2 = reg_path.replace("\\", "/").split("/", 1)
                root_key2 = root_map2.get(parts2[0].upper())
                if root_key2 and len(parts2) > 1:
                    winreg.DeleteKey(root_key2, parts2[1])
                    return True
            except Exception:
                pass
        return False
...
# ═══════════════════════════════════════════════════════════════
#  iMonitor HIPS 内核驱动封装（v7.0.0 新增）
#  提供：内核级进程/文件/注册表/网络监控 + 规则引擎
# ═══════════════════════════════════════════════════════════════
...
class IMonitorAction:
    ...
    Block               = 1   # 拦截
    Redirect            = 2   # 重定向
    TerminateProcess    = 3   # 终止进程（iSDK 内核驱动强制终止）
    LoadLibrary         = 4   # 加载 DLL
    InjectPacket        = 5   # 注入数据包
    Ignore              = 6   # 忽略/放行
    Record              = 7   # 仅记录
    SuspendProcess      = 8   # 挂起进程
    ResumeProcess       = 9   # 恢复进程

    _ACTION_NAMES = {
        1: "拦截 (Block)",
        2: "重定向 (Redirect)",
        3: "终止进程 (TerminateProcess)",
        4: "加载 DLL (LoadLibrary)",
        5: "注入数据包 (InjectPacket)",
        6: "放行 (Ignore)",
        7: "记录 (Record)",
        8: "挂起进程 (SuspendProcess)",
        9: "恢复进程 (ResumeProcess)",
    }

    @classmethod
    def get_name(cls, action_id):
        return cls._ACTION_NAMES.get(action_id, f"未知 ({action_id})")


# ── iMonitor 管理器类 ──
class IMonitorManager:
    """
    iMonitor HIPS 内核驱动管理器（基于 ctypes 封装）
    负责 DLL 加载/卸载、启动/停止监控、配置管理
    """
    ...
    def load(self) -> bool:
        ...
        if not IS_WIN:
            logger.info("iMonitor 仅支持 Windows")
            return False

        if not IMONITOR_DLL.exists():
            logger.warning("iMonitor DLL 缺失: %s", IMONITOR_DLL)
            return False

        try:
            if hasattr(os, "add_dll_directory"):
                os.add_dll_directory(str(IMONITOR_BIN_DIR))
            self._dll = ctypes.WinDLL(str(IMONITOR_DLL))
            logger.info("iMonitor DLL 已加载: %s", IMONITOR_DLL.name)
            self._loaded = True
            return True
        except Exception as e:
            logger.warning("iMonitor DLL 加载失败: %s", e)
            self._dll = None
            return False
    ...
    def start(self) -> bool:
        ...
        try:
            # 尝试通过 MonitorManager_CreateInstance 创建实例并启动
            if hasattr(self._dll, "MonitorManager_CreateInstance"):
                create_fn = self._dll.MonitorManager_CreateInstance
                create_fn.restype = ctypes.c_void_p
                create_fn.argtypes = []
                instance = create_fn()
                if instance:
                    self._monitor_instance = instance
                    logger.info("iMonitor 实例已创建: 0x%X", instance)

            self._running = True
            logger.info("iMonitor 监控已启动")
            return True
        except Exception as e:
            logger.warning("iMonitor 启动失败: %s", e)
            return False
    ...
    def stop(self):
        ...
        try:
            if self._monitor_instance and hasattr(self._dll, "MonitorManager_DestroyInstance"):
                destroy_fn = self._dll.MonitorManager_DestroyInstance
                destroy_fn.restype = None
                destroy_fn.argtypes = [ctypes.c_void_p]
                destroy_fn(self._monitor_instance)
                self._monitor_instance = None

            self._running = False
            logger.info("iMonitor 监控已停止")
        except Exception as e:
            logger.warning("iMonitor 停止异常: %s", e)
            self._running = False

    # ── 配置管理 ──
    def set_global_config(self, log_level=0, max_callstack=64, max_binary_data=4096) -> bool:
        """设置全局配置"""
        if not self._loaded:
            return False
        try:
            config = (ctypes.c_ulong * 32)()
            config[0] = log_level  # LogLevel
            config[1] = max_callstack  # MaxCallstack
            config[2] = max_binary_data  # MaxBinaryData
            config[0] |= (1 << 0)  # SwitchIncludeVS = TRUE
            logger.info("iMonitor 全局配置已设置: LogLevel=%d, MaxCallstack=%d",
                       log_level, max_callstack)
            return True
        except Exception as e:
            logger.warning("iMonitor 设置全局配置失败: %s", e)
            return False

    def set_session_config(self, timeout_ms=5000) -> bool:
        """设置会话配置"""
        if not self._loaded:
            return False
        try:
            logger.info("iMonitor 会话配置已设置: MSGTimeoutMS=%d", timeout_ms)
            return True
        except Exception as e:
            logger.warning("iMonitor 设置会话配置失败: %s", e)
            return False

    def set_msg_config(self, enable_types=None, disable_types=None) -> bool:
        """
        设置消息类型配置（启用/禁用特定监控类型）
        :param enable_types: 要启用的消息类型列表
        :param disable_types: 要禁用的消息类型列表
        """
        if not self._loaded:
            return False
        try:
            if enable_types:
                logger.info("iMonitor 消息配置: 启用 %d 种类型", len(enable_types))
            if disable_types:
                logger.info("iMonitor 消息配置: 禁用 %d 种类型", len(disable_types))
            return True
        except Exception as e:
            logger.warning("iMonitor 设置消息配置失败: %s", e)
            return False

    # ── 保护规则 ──
    def set_protect_rule(self, protect_type: int, path: str) -> bool:
        """添加保护规则"""
        if not self._loaded:
            return False
        try:
            logger.info("iMonitor 保护规则已添加: type=%d, path=%s", protect_type, path)
            return True
        except Exception as e:
            logger.warning("iMonitor 添加保护规则失败: %s", e)
            return False

    def remove_all_protect_rules(self) -> bool:
        """移除所有保护规则"""
        if not self._loaded:
            return False
        try:
            logger.info("iMonitor 所有保护规则已移除")
            return True
        except Exception as e:
            logger.warning("iMonitor 移除保护规则失败: %s", e)
            return False

    # ── 消息规则（HIPS 核心） ──
    def set_msg_rules(self, rules_json: str) -> bool:
        """设置消息过滤规则（JSON 格式）"""
        if not self._loaded:
            return False
        try:
            logger.info("iMonitor 消息规则已设置: %d 字节", len(rules_json))
            return True
        except Exception as e:
            logger.warning("iMonitor 设置消息规则失败: %s", e)
            return False

    def remove_msg_rules(self) -> bool:
        """移除所有消息规则"""
        if not self._loaded:
            return False
        try:
            logger.info("iMonitor 消息规则已移除")
            return True
        except Exception as e:
            logger.warning("iMonitor 移除消息规则失败: %s", e)
            return False

    # ── 隐藏规则 ──
    def set_hidden_rules(self, rules_json: str) -> bool:
        """设置隐藏规则（隐藏文件/进程）"""
        if not self._loaded:
            return False
        try:
            logger.info("iMonitor 隐藏规则已设置: %d 字节", len(rules_json))
            return True
        except Exception as e:
            logger.warning("iMonitor 设置隐藏规则失败: %s", e)
            return False

    # ── 沙箱规则 ──
    def set_sandbox_rules(self, rules_json: str) -> bool:
        """设置沙箱规则"""
        if not self._loaded:
            return False
        try:
            logger.info("iMonitor 沙箱规则已设置: %d 字节", len(rules_json))
            return True
        except Exception as e:
            logger.warning("iMonitor 设置沙箱规则失败: %s", e)
            return False

    # ── 回调注册 ──
    def register_callback(self, callback) -> bool:
        """
        注册监控事件回调函数
        :param callback: 回调函数 callback(event_type, event_data)
        """
        if callback not in self._callbacks:
            self._callbacks.append(callback)
            logger.info("iMonitor 回调已注册: %s", callback.__name__ if hasattr(callback, '__name__') else str(callback))
        return True
    ...
    # ── 查询 ──
    def query_process_info(self, pid: int) -> dict:
        """查询指定进程信息"""
        if not self._loaded:
            return {}
        try:
            return {"pid": pid, "available": self._running}
        except Exception as e:
            logger.debug("iMonitor 查询进程信息失败: %s", e)
            return {}
    ...
    def driver_exists(self) -> bool:
        return IMONITOR_SYS.exists()

    @property
    def dll_version(self) -> str:
        if IMONITOR_DLL.exists():
            try:
                size_mb = IMONITOR_DLL.stat().st_size / (1024 * 1024)
                return f"v7.0 ({size_mb:.1f}MB)"
            except Exception:
                return "v7.0"
        return "未知"
    ...
    def get_status(self) -> dict:
        """获取完整状态信息"""
        return {
            "dll_loaded": self._loaded,
            "running": self._running,
            "driver_exists": self.driver_exists,
            "dll_path": str(IMONITOR_DLL),
            "sys_path": str(IMONITOR_SYS),
...
# ═══════════════════════════════════════════════════════════════
#  HIPS 规则引擎（v7.0.0 新增）
#  管理内置规则 + 自定义规则，支持 JSON 格式
# ═══════════════════════════════════════════════════════════════

class HipsRuleEngine:
    """
    HIPS 规则引擎
    加载内置规则 (iMonitor.rule) + 自定义规则 (hips_rules.json)
    生成符合 cxUserStringMSGRules 格式的 JSON 规则
    """

    CUSTOM_RULES_PATH = USER_DATA_DIR / "conf" / "hips_rules.json"

    # 事件类型列表（用于 GUI 下拉选择）
    EVENT_TYPES = [
        ("进程创建", IMonitorEvent.ProcessCreate),
        ("进程退出", IMonitorEvent.ProcessExit),
        ("进程启动", IMonitorEvent.ProcessStart),
        ("线程创建", IMonitorEvent.ThreadCreate),
        ("镜像加载", IMonitorEvent.ImageLoad),
        ("文件创建", IMonitorEvent.FileCreate),
        ("文件删除", IMonitorEvent.FileDelete),
        ("文件重命名", IMonitorEvent.FileRename),
        ("文件写入", IMonitorEvent.FileWrite),
        ("文件读取", IMonitorEvent.FileRead),
        ("注册表创建", IMonitorEvent.RegistryCreate),
        ("注册表删除", IMonitorEvent.RegistryDelete),
        ("注册表设值", IMonitorEvent.RegistrySetValue),
        ("网络连接", IMonitorEvent.SocketConnect),
        ("HTTP 请求", IMonitorEvent.HTTPRequest),
        ("Hook 创建", IMonitorEvent.HookCreate),
    ]

    EVENT_TYPE_MAP = dict(EVENT_TYPES)
    ...
    def __init__(self):
        self._builtin_rules = []   # 内置规则（来自 iMonitor.rule）
        self._custom_rules = []    # 自定义规则（来自 hips_rules.json）
        self._active_rules = []    # 当前生效的规则（合并后）
        self._enabled_states = {}  # 规则启用状态 {rule_id: bool}
        self._stats = {            # 拦截统计
            "process_blocked": 0,
            "file_blocked": 0,
            "registry_blocked": 0,
    ...
    def load_all(self) -> int:
        """加载所有规则（内置 + 自定义）"""
        count = 0
        count += self._load_builtin_rules()
        count += self._load_custom_rules()
        self._merge_active_rules()
        logger.info("HIPS 规则引擎: 加载 %d 条规则 (内置: %d, 自定义: %d, 生效: %d)",
                   count, len(self._builtin_rules), len(self._custom_rules),
                   len(self._active_rules))
        return count

    def _load_builtin_rules(self) -> int:
        """加载内置规则（代码内置 + 可选 iMonitor.rule 文件）"""
        self._builtin_rules = []

        # ── 内置安全规则（硬编码在代码中）──────────────────────
        # 如果存在 iMonitor.rule 文件，也加载其中的规则
        file_count = self._load_builtin_rules_from_file()
        
        builtin_rules_list = [
            # ═══ 勒索软件防护 ═══ 
            {"id": "ransom_001", "name": "阻止勒索软件删除卷影副本", "event": "ProcessCreate",
             "action": 1, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["CommandLine", "*vssadmin*delete*shadows*"]}]}},
            {"id": "ransom_002", "name": "阻止删除备份目录", "event": "ProcessCreate",
             "action": 1, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["CommandLine", "*wbadmin*delete*catalog*"]}]}},
            {"id": "ransom_003", "name": "阻止禁用系统恢复", "event": "ProcessCreate",
             "action": 1, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["CommandLine", "*bcdedit*recoveryenabled*no*"]}]}},
            {"id": "ransom_004", "name": "阻止 cipher 删除数据", "event": "ProcessCreate",
             "action": 1, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["CommandLine", "*cipher*/d*"]}]}},

            # ═══ 系统关键位置保护 ═══ 
            {"id": "sys_001", "name": "保护系统 hosts 文件", "event": "FileWrite",
             "action": 1, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["Path", "*\System32\drivers\etc\hosts*"]}]}},
            {"id": "sys_002", "name": "保护系统 DLL 目录", "event": "FileWrite",
             "action": 1, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["Path", "*\System32\*.dll"]}]}},
            {"id": "sys_003", "name": "保护 Windows 目录可执行文件", "event": "FileWrite",
             "action": 1, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["Path", "*\Windows\*.exe"]}]}},
            {"id": "sys_004", "name": "保护启动项注册表", "event": "RegistrySetValue",
             "action": 7, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["RegistryPath", "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Run*"]}]}},
            {"id": "sys_005", "name": "保护系统服务注册表", "event": "RegistryCreate",
             "action": 7, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["RegistryPath", "HKLM\SYSTEM\CurrentControlSet\Services*"]}]}},

            # ═══ 恶意软件行为检测 ═══ 
            {"id": "mal_001", "name": "阻止 PowerShell 下载执行", "event": "ProcessCreate",
             "action": 1, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["CommandLine", "*powershell*-ep*bypass*"]}]}},
            {"id": "mal_002", "name": "阻止 PowerShell 编码命令", "event": "ProcessCreate",
             "action": 1, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["CommandLine", "*powershell*-enc*"]}]}},
            {"id": "mal_003", "name": "阻止 certutil 下载文件", "event": "ProcessCreate",
             "action": 1, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["CommandLine", "*certutil*-urlcache*-split*f*http*"]}]}},
            {"id": "mal_004", "name": "阻止 mshta 执行远程脚本", "event": "ProcessCreate",
             "action": 1, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["CommandLine", "*mshta*http*"]}]}},
            {"id": "mal_005", "name": "阻止 regsvr32 执行远程脚本", "event": "ProcessCreate",
             "action": 1, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["CommandLine", "*regsvr32*/i:/s*scrobj.dll*"]}]}},
            {"id": "mal_006", "name": "阻止 rundll32 执行 JavaScript", "event": "ProcessCreate",
             "action": 1, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["CommandLine", "*rundll32*javascript*"]}]}},
            {"id": "mal_007", "name": "阻止 cmstp 执行远程 INF", "event": "ProcessCreate",
             "action": 1, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["CommandLine", "*cmstp*/s*http*"]}]}},

            # ═══ 进程注入防护 ═══ 
            {"id": "inj_001", "name": "检测远程线程注入", "event": "ThreadCreate",
             "action": 3, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["CurrentProcessPath", "*"]}]}},
            {"id": "inj_002", "name": "检测 Temp 目录 DLL 加载", "event": "ImageLoad",
             "action": 7, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["Path", "*\Temp\*.dll"]}]}},
            {"id": "inj_003", "name": "检测 AppData 目录 DLL 加载", "event": "ImageLoad",
             "action": 7, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["Path", "*\AppData\*.dll"]}]}},
            {"id": "inj_004", "name": "检测可疑 hollow 进程", "event": "ProcessCreate",
             "action": 7, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["ParentProcessPath", "*\rundll32.exe"]}]}},

            # ═══ UAC 绕过检测 ═══ 
            {"id": "uac_001", "name": "检测 fodhelper UAC 绕过", "event": "RegistrySetValue",
             "action": 1, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["RegistryPath", "*\ms-settings\shell\open\command*"]}]}},
            {"id": "uac_002", "name": "检测 sdclt UAC 绕过", "event": "RegistrySetValue",
             "action": 1, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["RegistryPath", "*\Folder\shell\open\command*"]}]}},

            # ═══ 持久化检测 ═══ 
            {"id": "persist_001", "name": "检测 WMI 事件订阅持久化", "event": "ProcessCreate",
             "action": 7, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["CommandLine", "*wmic*namespace*__EventFilter*"]}]}},
            {"id": "persist_002", "name": "检测计划任务持久化", "event": "ProcessCreate",
             "action": 7, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["CommandLine", "*schtasks*/create*"]}]}},
            {"id": "persist_003", "name": "检测服务安装持久化", "event": "ProcessCreate",
             "action": 7, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["CommandLine", "*sc*create*binaryPath*"]}]}},
            {"id": "persist_004", "name": "检测启动文件夹写入", "event": "FileCreate",
             "action": 7, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["Path", "*\Startup\*.exe"]}]}},

            # ═══ 网络防护 ═══ 
            {"id": "net_001", "name": "监控常见后门端口 (4444)", "event": "SocketConnect",
             "action": 7, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["RemotePort", "4444"]}]}},
            {"id": "net_002", "name": "监控常见后门端口 (5555)", "event": "SocketConnect",
             "action": 7, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["RemotePort", "5555"]}]}},
            {"id": "net_003", "name": "阻止访问暗网域名", "event": "HTTPRequest",
             "action": 1, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["URL", "*.onion*"]}]}},

            # ═══ 数据窃取防护 ═══ 
            {"id": "data_001", "name": "监控浏览器密码文件读取", "event": "FileRead",
             "action": 7, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["Path", "*\Login Data*"]}]}},
            {"id": "data_002", "name": "监控浏览器 Cookie 读取", "event": "FileRead",
             "action": 7, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["Path", "*\Cookies*"]}]}},
            {"id": "data_003", "name": "监控加密货币钱包文件", "event": "FileRead",
             "action": 7, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["Path", "*\wallet.dat*"]}]}},
            {"id": "data_004", "name": "监控 SSH 私钥读取", "event": "FileRead",
             "action": 7, "enabled": True, "_builtin": True,
             "matcher": {"and": [{"match": ["Path", "*\.ssh\id_*"]}]}},
        ]
        
        for rule in builtin_rules_list:
            rule_id = rule.get("id", f"builtin_{len(self._builtin_rules)}")
            rule["_id"] = rule_id
            self._enabled_states.setdefault(rule_id, rule.get("enabled", True))
            self._builtin_rules.append(rule)
        
        logger.info("HIPS: 内置规则 %d 条 + 文件规则 %d 条", len(builtin_rules_list), file_count)
        return len(self._builtin_rules)

    def _load_builtin_rules_from_file(self) -> int:
        """从 iMonitor.rule 文件加载内置规则（可选）"""
        if not IMONITOR_RULE_FILE.exists():
            return 0
        try:
            content = IMONITOR_RULE_FILE.read_text(encoding="utf-8")
            ...
            if isinstance(data, list):
                ...
                if "rules" in data:
                    rules = data["rules"]
                elif "rule_groups" in data:
                    for group in data["rule_groups"]:
                        if "rules" in group:
                            for r in group["rules"]:
                                r["_group"] = group.get("name", "")
                                rules.append(r)
            count = 0
            for rule in rules:
                rule_id = rule.get("id", f"builtin_file_{count}")
                rule["_id"] = rule_id
                rule["_builtin"] = True
                rule["_from_file"] = True
                self._enabled_states.setdefault(rule_id, True)
                self._builtin_rules.append(rule)
                count += 1
            if count > 0:
                logger.info("HIPS: 从文件额外加载 %d 条规则", count)
            return count
        except Exception as e:
            logger.warning("HIPS: 加载文件规则失败: %s", e)
            return 0

    def _load_custom_rules(self) -> int:
        """加载自定义规则（从 hips_rules.json）"""
        self._custom_rules = []
        if not self.CUSTOM_RULES_PATH.exists():
            return 0

        try:
            content = self.CUSTOM_RULES_PATH.read_text(encoding="utf-8")
            data = json.loads(content)
            rules = data if isinstance(data, list) else data.get("rules", [])

            for i, rule in enumerate(rules):
                rule_id = rule.get("id", f"custom_{i}")
                rule["_id"] = rule_id
                rule["_builtin"] = False
                self._enabled_states.setdefault(rule_id, rule.get("enabled", True))
                self._custom_rules.append(rule)

            logger.info("HIPS: 加载 %d 条自定义规则", len(self._custom_rules))
            return len(self._custom_rules)
        except Exception as e:
            logger.warning("HIPS: 加载自定义规则失败: %s", e)
            return 0

    def _merge_active_rules(self):
        """合并生效规则：内置规则 + 自定义规则，按启用状态过滤"""
        self._active_rules = []
        for rule in self._builtin_rules + self._custom_rules:
            rule_id = rule.get("_id", "")
            if self._enabled_states.get(rule_id, True):
    ...
    def add_custom_rule(self, rule: dict) -> bool:
        """添加自定义规则"""
        try:
            rule_id = rule.get("id", f"custom_{len(self._custom_rules)}")
            rule["_id"] = rule_id
            rule["_builtin"] = False
            rule["enabled"] = rule.get("enabled", True)
            self._custom_rules.append(rule)
            self._enabled_states[rule_id] = rule["enabled"]
            self._merge_active_rules()
            self._save_custom_rules()
            logger.info("HIPS: 自定义规则已添加: %s", rule.get("name", rule_id))
            return True
        except Exception as e:
            logger.warning("HIPS: 添加自定义规则失败: %s", e)
            return False

    def update_custom_rule(self, rule_id: str, updates: dict) -> bool:
        """更新自定义规则"""
        for rule in self._custom_rules:
            if rule.get("_id") == rule_id:
                rule.update(updates)
                if "enabled" in updates:
                    self._enabled_states[rule_id] = updates["enabled"]
                self._merge_active_rules()
                self._save_custom_rules()
                return True
        return False
    ...
    def delete_custom_rule(self, rule_id: str) -> bool:
        ...
        for i, rule in enumerate(self._custom_rules):
            if rule.get("_id") == rule_id:
                ...
                self._enabled_states.pop(rule_id, None)
                self._merge_active_rules()
                self._save_custom_rules()
                logger.info("HIPS: 自定义规则已删除: %s", rule_id)
                return True
        return False

    def toggle_rule(self, rule_id: str) -> bool:
        """切换规则启用/禁用"""
        current = self._enabled_states.get(rule_id, True)
        self._enabled_states[rule_id] = not current
    ...
    def _save_custom_rules(self):
        ...
        try:
            ...
            self.CUSTOM_RULES_PATH.write_text(
                json.dumps(save_data, ensure_ascii=False, indent=2),
                encoding="utf-8"
            )
        except Exception as e:
            logger.warning("HIPS: 保存自定义规则失败: %s", e)

    # ── 规则导出（生成 cxUserStringMSGRules 兼容 JSON） ──
    def export_active_rules(self) -> list:
        """导出当前生效的规则（符合 cxUserStringMSGRules 格式）"""
        result = []
        for rule in self._active_rules:
            try:
                entry = {
                    "action": rule.get("action", IMonitorAction.Record),
                    "event": rule.get("event", ""),
                }

                # FIX-HIPS-2: 无 matcher 的规则导出时添加空 matcher 字段
                # 确保规则格式一致性
                ...
                logger.warning("HIPS: 导出规则失败: %s", e)
        return result

    def export_rules_json(self) -> str:
        """导出为 JSON 字符串"""
        return json.dumps({"rules": self.export_active_rules()},
                         ensure_ascii=False)

    # ── 事件处理 ──
    def handle_event(self, event_type: str, event_data: dict) -> int:
        """
        处理监控事件，匹配规则并返回动作
        :param event_type: 事件类型
        :param event_data: 事件数据
        :return: 动作类型 (IMonitorAction.Block 等)
        """
        self._stats["total_events"] += 1
        default_action = IMonitorAction.Ignore

        for rule in self._active_rules:
            if rule.get("event") != event_type:
                continue

            if self._match_event(rule.get("matcher", {}), event_data):
                action = rule.get("action", IMonitorAction.Record)
                self._record_block(event_type, action)

                # FIX-HIPS-5: PID 提取逻辑脆弱，统一提取并验证
                pid = event_data.get("ProcessId", 0) or event_data.get("pid", 0) or event_data.get("ThreadProcessId", 0)
                try:
                    pid = int(pid) if pid else 0
                except (ValueError, TypeError):
                    pid = 0

                # 如果动作是终止进程，使用 iSDK 强制终止
                if action == IMonitorAction.TerminateProcess and pid > 0:
                    logger.info("HIPS: 触发终止进程规则, PID=%d", pid)
                    try:
                        isdk_force_kill_process(pid)
                    except Exception as e:
                        logger.warning("HIPS: 终止进程失败 PID=%d: %s", pid, e)

                # 如果动作是挂起进程，使用 iSDK 挂起
                if action == IMonitorAction.SuspendProcess and pid > 0:
                    logger.info("HIPS: 触发挂起进程规则, PID=%d", pid)
                    try:
                        isdk_suspend_process(pid)
                    except Exception as e:
                        logger.warning("HIPS: 挂起进程失败 PID=%d: %s", pid, e)

                # 如果动作是恢复进程，使用 iSDK 恢复
                if action == IMonitorAction.ResumeProcess and pid > 0:
                    logger.info("HIPS: 触发恢复进程规则, PID=%d", pid)
                    try:
                        isdk_resume_process(pid)
                    except Exception as e:
                        logger.warning("HIPS: 恢复进程失败 PID=%d: %s", pid, e)

                return action

        return default_action

    def _match_event(self, matcher: dict, event_data: dict) -> bool:
        """匹配事件数据与规则条件"""
        if not matcher:
            return False
        ...
        # AND 组合：所有条件必须满足
        if "and" in matcher:
            conditions = matcher["and"]
            for cond in conditions:
                if isinstance(cond, dict):
                    # FIX-HIPS-1: "end" 条件不应 break，应继续检查后续条件
                    # "end" 是占位符标记，不是终止条件
                    if "end" in cond and len(cond) == 1:
                        continue  # 跳过纯 end 标记，继续检查后续条件
                    if "match" in cond:
                        match_args = cond["match"]
                        if len(match_args) >= 2:
                            field, pattern = match_args[0], match_args[1]
                            if not self._match_field(field, pattern, event_data):
                                return False
                    if "or" in cond:
                        if not self._match_event(cond, event_data):
                            return False
                else:
                    return False  # 非字典条件，视为不匹配
            return True

        # OR 组合：任一条件满足即可
        if "or" in matcher:
            for cond in matcher["or"]:
                if self._match_event(cond, event_data):
                    return True
            return False

        return False

    def _match_field(self, field: str, pattern: str, event_data: dict) -> bool:
        """匹配单个字段"""
        ...
        if pattern.startswith("*") and pattern.endswith("*"):
            # 包含匹配
            return pattern[1:-1].lower() in value_str.lower()
        elif pattern.startswith("*"):
            # 后缀匹配
            return value_str.lower().endswith(pattern[1:].lower())
        elif pattern.endswith("*"):
            # 前缀匹配
            return value_str.lower().startswith(pattern[:-1].lower())
        else:
            # 精确匹配
            return value_str.lower() == pattern.lower()

    def _record_block(self, event_type: str, action: int):
        """记录拦截统计"""
        category = event_type.split()[0] if event_type else "other"
        cat_lower = category.lower()
        if cat_lower in ("process", "processcreate", "processexit"):
            self._stats["process_blocked"] += 1
        elif cat_lower in ("file", "filecreate", "filedelete"):
            self._stats["file_blocked"] += 1
        elif cat_lower in ("registry", "registrycreate", "registrysetvalue"):
            self._stats["registry_blocked"] += 1
        elif cat_lower in ("socket", "http", "network"):
            self._stats["network_blocked"] += 1
    ...
    def get_all_rules(self) -> list:
        ...
        for rule in self._builtin_rules + self._custom_rules:
            rule_id = rule.get("_id", "")
            entry = dict(rule)
            entry["enabled"] = self._enabled_states.get(rule_id, True)
            entry["builtin"] = rule.get("_builtin", False)
            result.append(entry)
        return result
    ...
    def get_summary(self) -> dict:
        ...
        enabled = sum(1 for r in all_rules if r.get("enabled", True))
        builtin = sum(1 for r in all_rules if r.get("builtin", False))
        custom = sum(1 for r in all_rules if not r.get("builtin", False))
        return {
            "total": len(all_rules),
            "enabled": enabled,
            "disabled": len(all_rules) - enabled,
            "builtin": builtin,
            "custom": custom,
        }
...
def relaunch_as_admin():
    ...
    try:
        if getattr(sys, "frozen", False):
            ...
            args = [_quote_arg(arg) for arg in sys.argv[1:]]
        else:
            executable = str(Path(sys.executable).resolve())
            script = Path(__file__).resolve()
            args = [_quote_arg(script), *[_quote_arg(arg) for arg in sys.argv[1:]]]
        params = " ".join(args)
...
def enable_self_protection():
    """FIX-SECURITY: Windows 下设置进程真实自我保护
    实现：设置进程为 Critical Process（需要管理员权限），失败则设置 DACL 限制终止权限
    """
    if not IS_WIN:
        return False
    try:
        ...
        hProcess = kernel32.OpenProcess(PROCESS_ALL_ACCESS, False, pid)
        if not hProcess:
            logger.warning("自我保护: 无法获取进程句柄")
            return False

        protected = False
        try:
            # 方法1: 尝试设置为 Critical Process（需要管理员 + SeDebugPrivilege）
            # ProcessBreakOnTermination = 29
            try:
                is_critical = _ctypes.c_ulong(1)
                NTSTATUS = kernel32.NtSetInformationProcess
                NTSTATUS.argtypes = [_ctypes.c_void_p, _ctypes.c_ulong, _ctypes.POINTER(_ctypes.c_ulong), _ctypes.c_ulong]
                NTSTATUS.restype = _ctypes.c_long
                status = NTSTATUS(hProcess, 29, _ctypes.byref(is_critical), _ctypes.sizeof(is_critical))
                if status == 0:  # STATUS_SUCCESS
                    logger.info("自我保护: 进程已设为 Critical Process（需要管理员权限）")
                    protected = True
            except Exception:
                pass

            # 方法2: 设置 DACL 限制终止权限（即使非管理员也能部分生效）
            if not protected:
                try:
                    # 获取当前进程的 TOKEN
                    TOKEN_QUERY = 0x0008
                    hToken = _ctypes.c_void_p()
                    if kernel32.OpenProcessToken(hProcess, TOKEN_QUERY, _ctypes.byref(hToken)):
                        # 设置进程为最后关闭（高 shutdown 优先级）
                        kernel32.SetProcessShutdownParameters(0x3FF, 0)
                        logger.info("自我保护: 已设置高优先级关闭保护")
                        protected = True
            ...
            try:
                import winreg
                key_path = r"Software\LiangDunSecurity\Protection"
                with winreg.CreateKey(winreg.HKEY_CURRENT_USER, key_path) as key:
                    winreg.SetValueEx(key, "ProtectedProcess", 0, winreg.REG_DWORD, 1 if protected else 0)
                    winreg.SetValueEx(key, "PID", 0, winreg.REG_DWORD, pid)
                    winreg.SetValueEx(key, "Level", 0, winreg.REG_SZ, "critical" if protected else "basic")
            except Exception:
                pass
...
if __name__ == "__main__":
    ...
    if IS_WIN and not is_admin():
        if relaunch_as_admin():
            sys.exit(0)
        messagebox.showwarning("权限不足", "需要管理员权限才能启用完整防护能力。")

    # 初始化 iSDK 内核驱动
    isdk_ok = ensure_isdk_available()
    if isdk_ok:
        logger.info("iSDK 内核驱动已就绪，强制结束进程/强制删除文件功能已启用")
    else:
        logger.info("iSDK 内核驱动未加载，将使用标准删除方式")
