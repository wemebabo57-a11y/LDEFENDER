...
class LiangDun(ctk.CTk):
    ...
    def create_scan_tab(self):
        ...
        self.scan_path = ctk.CTkEntry(f, width=700, placeholder_text="选择文件/文件夹")
        self.scan_path.pack(pady=5, padx=10)

        browse_frame = ctk.CTkFrame(f, fg_color="transparent")
        browse_frame.pack(pady=5, padx=10)
        ctk.CTkButton(browse_frame, text="浏览文件", width=110, command=self.choose_scan_file).pack(side="left", padx=4)
        ctk.CTkButton(browse_frame, text="浏览文件夹", width=110, command=self.choose_scan_folder).pack(side="left", padx=4)
        ctk.CTkButton(browse_frame, text="多文件选择", width=110, command=self.choose_scan_files).pack(side="left", padx=4)
        self.scan_btn = ctk.CTkButton(browse_frame, text="开始扫描", width=120, fg_color="#E53935", command=self.start_scan)
        self.scan_btn.pack(side="left", padx=10)
        self.online_btn = ctk.CTkButton(browse_frame, text="在线查毒", width=120, fg_color="#165DFF", command=self.online_scan)
        self.online_btn.pack(side="left", padx=4)
        self.quarantine_btn = ctk.CTkButton(browse_frame, text="隔离沙盒", width=120, fg_color="#8E44AD", command=self.quarantine_files)
        self.quarantine_btn.pack(side="left", padx=4)

        self.scan_text = ctk.CTkTextbox(f, width=1050, height=320, font=("Consolas", 11))
        self.scan_text.pack(pady=10, padx=10, fill="both", expand=True)
    ...
    def choose_scan_files(self):
        paths = filedialog.askopenfilenames(title="选择多个文件")
        if paths:
            self.scan_path.delete(0, "end")
            self.scan_path.insert(0, ";".join(paths))

    def get_scan_targets(self, raw_path):
        raw_path = raw_path.strip()
        if not raw_path:
            return []
        if ";" in raw_path:
            targets = [p.strip() for p in raw_path.split(";") if p.strip()]
        else:
            targets = [raw_path]
        valid = []
        for p in targets:
    ...
    def iter_files_for_target(self, target):
    ...
    def start_scan(self):
        raw_path = self.scan_path.get().strip()
        targets = self.get_scan_targets(raw_path)
        if not targets:
            return messagebox.showwarning("提示", "请选择文件、文件夹或多个文件")
        ...
        def scan():
            ...
            for target in targets:
                if os.path.isfile(target):
                    ...
                    try:
                        for fp in self.iter_files_for_target(target):
                            cnt += 1
    ...
    def online_scan(self):
        ...
        def run():
            ...
            total = 0
            hits = 0
            logs.append(f"在线查毒目标: {' | '.join(targets)}\n\n")
            for target in targets:
                for fp in self.iter_files_for_target(target):
                    total += 1
                    try:
                        ...
                        try:
                            ...
                            res = {"结果": "在线查询失败"}
                        logs.append(f"文件: {fp}\nSHA256: {sha256}\n在线查询结果: {res}\n\n")
                        if isinstance(res, dict) and (res.get("known") is True or res.get("malicious") is True):
                            hits += 1
                    except PermissionError:
                        logs.append(f"文件: {fp}\n结果: 无权限读取\n\n")
                    except Exception:
                        logs.append(f"文件: {fp}\n结果: 无法读取或查询失败\n\n")
            logs.append(f"在线查毒完成 | 文件总数:{total} | 命中:{hits}")
            log = "".join(logs)
            self.ui_call(lambda: (
                self.scan_text.delete("1.0","end"),
                self.scan_text.insert("end", log),
    ...
    def get_quarantine_dir(self):
        qdir = os.path.join(os.path.dirname(sys.argv[0]), "quarantine_sandbox")
        os.makedirs(qdir, exist_ok=True)
        return qdir

    def record_quarantine_manifest(self, items):
        manifest_path = os.path.join(self.get_quarantine_dir(), "manifest.json")
        data = []
    ...
    def quarantine_files(self):
        selected = list(filedialog.askopenfilenames(title="选择要隔离的多个文件"))
        entry_value = self.scan_path.get().strip()
        if not selected and entry_value:
            if os.path.isfile(entry_value):
                selected = [entry_value]
        if not selected:
            return messagebox.showwarning("提示", "请选择一个或多个文件")
        ...
        for src in selected:
            try:
                ...
                dst_name = f"{stamp}_{base}"
                dst = os.path.join(qdir, dst_name)
                shutil.move(src, dst)
                results.append({
                    "source": src,
                    "quarantined": dst,
        ...
        if results:
            self.record_quarantine_manifest(results)

        msg = f"已隔离 {moved} 个文件"
        if failed:
            msg += f"，失败 {failed} 个"
        self.scan_text.insert("end", f"\n{msg}\n隔离目录: {qdir}\n")
        messagebox.showinfo("隔离完成", msg)
